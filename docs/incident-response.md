...
### 2026-07-10 → 2026-07-23 — archived-client rows API-writable until the DB write-immutability guards reach production (CLOSED)

- **What:** The CN-7 archived-client immutability guards + completed-session edit-lock (`docs/polish/db-write-immutability.md`, migration `20260721120000`) were built and verified on staging 2026-07-21 (revised 2026-07-22 after reviewer return), but production apply is deferred to the Step-7 deploy sitting. Confirmed live on staging *before* the guards: a same-org staff credential could direct-UPDATE (and, unguarded, hard-DELETE) an archived client's `clients` row, and could force-book / stale-tab-write archived-client child records via raw PostgREST. Until the migration applies to production, the same holes are open there. Raised by the reviewer 2026-07-22 (blocker 2) as an open production exposure that must be recorded, not filed as "incidental."
- **Window:** archived client rows have existed in production since the first archive (Imaan Sedghi moved to EXCO, 2026-07-10, plus any later archive) → the Step-7 prod apply (pending).
- **Exposure during the window:** low at f&f scope. The actor is a same-org authenticated staff credential (the operator + one trusted EP), not an outsider or client; the target is an already-archived record; the write path is raw PostgREST only (the app-layer `assertClientLive` guard already refuses these actions through the UI); every such write is audit-logged. No cross-tenant or client-facing surface.
- **Deadline:** the Step-7 deploy sitting — not open-ended. The guards ship at the next production deploy on operator instruction.
- **Closes by:** `supabase db push` of `20260721120000` to production + pgTAP 60 re-run green on prod, at which point every path above is refused by design.
- **CLOSED 2026-07-23 (AEST; 2026-07-22 ~22:05 UTC), the prod-apply sitting.** `20260721120000` applied to production via the prod-workdir channel (`supabase db push`, migration list zero-pending afterward). **Amendment to the "Closes by" line above:** the prod confirmation that ran is the one the write-immutability sign-off actually prescribes — round-2 follow-up 5 (`polish/db-write-immutability.md` §8, prod-apply precondition 3) deliberately downgraded "pgTAP 60 re-run on prod" to a **light smoke**, because test 60 is a write-heavy fixture against prod; this entry's older wording predated that decision and was not updated. What ran: (a) catalog census — all 9 `write_immutability_guard` triggers + the `auth_events` append-only trigger + `reminder_log_communication` present on prod; (b) CR-normalised `pg_get_functiondef` parity on all seven guard/RPC functions, prod ≡ staging (where the full pgTAP 60 runs 17/17); (c) live refusal smoke in a rolled-back transaction under `odyssey.test_enforce_guards='1'` — an archived client's edit AND hard-delete both refused with the exact app copy. One recorded deviation: the *pre-push* `pg_get_functiondef` snapshot (precondition 1) did not run — the sitting runbook's Phase-1 code block omitted the command its prose named — so the post-apply parity check (b) stands in for it; prod's functions were only ever touched by tracked migrations, and the applied end-state is byte-identical (modulo line endings) to the staging bodies the reviewer signed off.
