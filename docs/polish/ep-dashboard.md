...
**Status:** **Decisions resolved by the operator 2026-06-22 (see §5) — gap list amended.** Protocol steps 1–4 complete; the gap list (§3, as amended by §5) is the contract. **P1-1 (client list) is WITHDRAWN at the operator's request — a deliberate UX/UI decision, not an implementation miss (§5).** Awaiting the operator's final go-ahead to begin building (and a nod on the P2-7 mock). **No code has been changed.**
...
| **FM-9** | **No responsive layout.** Stat grid is a fixed inline `repeat(4,1fr)` and the body is a fixed `2fr 1fr` (`page.tsx:284-343`) — inline styles can't carry media queries, so nothing collapses. Brief §8 requires the staff portal responsive down to 768px (the prototype collapsed 4→2→1). | Certain < ~900px | Cramped/overflow on tablet; desktop-first lowers severity | **P2-4** |
...
| **P2-3** | **Token / inline-literal sweep.** | FM-8 | Promote the repeated stat-card / panel / row inline literals to design-system classes or `var(--…)` tokens (`page.tsx:284-343` and throughout). Same treatment as scheduling P2-13. No raw colours/radii/spacing introduced. |
| **P2-4** | **Responsive breakpoints (down to 768px).** | FM-9 | Give the stat grid and the body layout real breakpoints (4→2→1 stat cards; sidebar drops below the main column on narrow widths), per brief §8. Inline grids → a styled container so media queries apply. Desktop-first, so lowest-priority P2. |
