...
**Delivered (2026-06-24) — full parity + small backend.** The "no new backend" claim above was wrong: the spec's own copy-list (Instructions, rest/tempo, up/down) had no circuit backing — `circuit-actions.ts` only covered the set table, and the loader fetched `id, name` for the exercise. Surfaced to the operator pre-code; **decision: full parity (the stated goal is "look exactly like the builder"), add the small backend** (the columns already exist on `circuit_exercises` from `20260624100000`, so **no migration**). Shipped on `feat/library-circuits`:

- **Backend — 2 actions** in `circuit-actions.ts`, mirroring the program equivalents: `updateCircuitExerciseAction` (allowlisted scalar patch — `instructions`/`rest_seconds`/`tempo`; no `section_title`, a circuit is one group) and `moveCircuitExerciseAction` (↑/↓ — adjacent `sort_order` value-swap; no superset re-derivation to do, so not the builder's reorder RPC). Both RLS staff-UPDATE via parent, no soft-delete trap (never touch `deleted_at`).
- **`CircuitEditor.tsx`** — rewritten as the carbon-copy: two-column card (left: plain name + ↑/↓/delete + Instructions `EditableTextarea` + 96×60 video tile; right: `SetTable` with `VolumeColumnDropdown`/`MetricColumnDropdown` black slabs + `SetStepper` + `ExtrasRow`), `SetCell` autosave with the idle/saving/error revert + `SaveTick`. Token aliases (`INK`…`GREEN`) copied verbatim. `SessionBuilder.tsx` untouched.
- **One group spine.** All exercises render under a single continuous slate spine with **A1, A2…** letters (the literal "a circuit *is* a group" reading), even a single exercise (A1) — not the builder's solo-pill-for-one. Flag for the `:3000` pass if the single-exercise spine reads oddly (trivial to switch to a solo pill).
- **`CircuitLibraryPanel.tsx`** (new) — the builder's `LibraryPanel` **cloned**, not generalised (per the standing "copy, don't touch the differentiator" rule). Same shared atoms (`SearchInput`/`PatternChips`/`TagChips`/`ExerciseGrid`), wired to `addExerciseToCircuitAction` (append). No insert-slot/swap machinery — picking always appends.
- **`page.tsx`** — loader expanded: `circuit_exercises` now selects `rest_seconds`/`tempo`/`instructions`, the exercise join adds `video_url`; plus the full `LIBRARY_EXERCISE_COLUMNS` card shape, `movement_patterns`, `exercise_tags`, and `exercise_metric_units` for the panel + the LOAD dropdown. Page widened to `maxWidth: 1320` (two-column).

**Acceptance:** `type-check` clean; `npm run build` **green** (route `/library/circuits/[id]` present). No DB change → **no new pgTAP** (the two actions are RLS staff-writes over the already-tested `39`/`40` surface; the anon-revoke posture is unchanged). Render-tier — the card paint, the dropdowns, ↑/↓ reorder, the Library-panel chips/append — is **operator `:3000`** per `go-live-checklist.md §5b` (staff-auth-gated). Verify loop: open a circuit → confirm the card matches the builder → edit reps/load/measure/rest/tempo/instructions (watch the save tick) → ↑/↓ reorder → add an exercise from the right panel → remove one.
