...
**What this does to the C-14 scanner-class weakness:** with no link in the HTML (C-11) and now no link minted before the human acts, a body-parsing scanner has nothing to reach — **conditional on the C-11 no-POST premise** (scanners GET the gate; they do not execute its form POST). That premise's enterprise validation — the **Safe Links/M365 behavioural re-run** (checklist §8) — remains **outstanding** at the paying-client gate (it needs an M365 mailbox), so the claim is not yet behaviourally confirmed against the corporate-scanner class. And note the boundary C-14 does *not* move: `/i/<id>` is a **bearer capability** — anyone holding the gate URL can POST it and mint the OTP — so C-14 narrows the *scanner* class (automated body-parsing prefetch), not the *interception* class (anyone who obtains the URL).

---

## Ledger reconciliation — 2026-07-22 (platform drift audit)

Appended by the ten-doc ledger drift audit run out of the staff-auth reviewer pass (the single-ledger rule now in `go-live-checklist.md`'s header is its product). The sections above predate the following closures and were not re-edited; this note is the reconciliation. **Pro-plan gate items CLOSED 2026-07-21** at the Pro cutover: HIBP ON and GREEN behaviourally, **G-3u GREEN — the definitive C-7 answer** (HIBP fires on `updateUser`; C-7 closes verified, no code change), and the session time-box set to 30 days (`go-live-checklist.md` §1/§7; `runbooks/verify-auth-config.md` 2026-07-21 run-log row). **Open question 1 (client session duration) DECIDED 2026-07-21** — 30-day uniform across roles at f&f scope, revisit at the paying-client gate (checklist §1). **C-1 branch-(b) / C-6 infra-error runtime verification:** the enabling trigger — a non-production Supabase target — FIRED 2026-07-03 (staging `odyssey-staging`, checklist §5); the verification itself has **not yet run** and is now unblocked — this is the one actionable outstanding item this doc still owns (indexed on `go-live-checklist.md` §8, 2026-07-22, per the single-ledger rule). The tooling/feature residuals (rate-limit wiring, profile-sync guard, "Pending" backfill, `/portal/you` sign-out, unit-test ports) remain doc-local feature deferrals. This doc is a historical record per the single-ledger rule; current state lives on the checklist.

---

## Post-hoc reviewer pass discharged (2026-07-22) — the three actionables run same-day; pass complete

**Context.** Following the 2026-07-22 ledger reconciliation above, the claude.ai reviewer delivered a post-hoc verdict on this section's close: **"sound to close — the close itself was correct in 2026-06-11 terms,"** with three actionable items and two structural notes. This entry records all three actionables discharged the same day. Work ran on branch `client-auth-signoff-closeout` (stacked per convention); every database operation targeted **staging** (`odyssey-staging` `fbtfzlgvnivgwydlijka`, resolved from the CLI linked ref `supabase/.temp/project-ref` and the `.env.local` default keys per the environment-separation rules); production was not touched.

### 1. C-1 branch-(b) + C-6 infra-error runtime verification — RUN against staging, every branch observed

The one actionable item this doc still owned (trigger fired 2026-07-03; unrun for nineteen days — the reviewer's firing-discipline point is taken).

**C-1 (welcome-flow recovery — closed 2026-06-08 by exhaustive reading, runtime-blocked until a non-prod target existed).** The stuck state was reproduced faithfully with no code break and no prod DML — the reproduction the original closure said was impossible against the live project: create a probe auth user + unlinked clients row (`scripts/c1-c6-staging-verify.mjs c1-setup`, committed as the re-runnable harness), sign in through the real `/login` form — the hook mints a **claimless** JWT because no membership exists yet — then inject the post-RPC state server-side (`c1-link`: membership row + `clients.user_id` + `onboarded_at`) while the browser still holds the stale JWT. That is byte-for-byte the "client_accept_invite succeeded, refreshSession() failed" state. Observed, in order:

- **Branch (c), pre-link:** `/welcome/install` redirected to `/welcome`'s "Something's missing." terminus — verbatim copy confirmed.
- **The R-5 dead-end shape:** a claimless login lands on `/unauthorized` (postAuthLanding → portal-layout bounce), as documented at C-1/C-4.
- **Branch (b), happy:** post-link, `/welcome/install` rendered FinishSetup and self-healed — proven by the `odyssey_c1_recovery_at` sessionStorage stamp (written only by FinishSetup, only after its single successful refresh) and the hard landing on `/portal` rendering "Good evening, C1." through real RLS reads. The soft-lockout → bounded-self-heal conversion is now observed, not asserted.
- **Branch (b), failed:** with the refresh token poisoned inside the auth cookie (access token left intact, so the server still rendered branch (b)), FinishSetup stopped at exactly one attempt and rendered the failed state — "Finish setting up" / "Your invite was accepted. Sign out and sign back in to load your portal." / Sign out — no loop, no indefinite spinner.
- **The escape:** the Sign out button signed out for real — landed on `/` with the auth cookie cleared.

*Honest scope:* the refresh failure was induced by refresh-token poisoning — a faithful stand-in for "refreshSession() returns an error," not for a network hang; the FinishSetup infinite-hang sliver recorded at the C-1 closure remains accepted, unchanged. The 30-second re-bounce guard's immediate-failed path also remains reading-verified: the state it guards (/portal bouncing back within 30s of a *successful* refresh) cannot be composed once the claims are real.

**C-6 (the two deliberately-opposite infra-error branches — closed 2026-06-08 with these branches reading-verified).** A real infrastructure failure was induced on staging: `REVOKE EXECUTE … FROM authenticated` on `rate_limit_check_and_record` + `rate_limit_check_failures` (ACL recorded before/after; scoped to `authenticated` only, so service-role work — including any concurrent session's — was never at risk). Observed:

- **Fail open (staff gate):** with the RPC dead, the real "Invite client" flow admitted end-to-end — client row created, invite-token row written (`action_link` NULL per mint-at-POST), redirect to the new profile — with the exact branch log: `[rate-limit] staff_invite check failed (failing open): permission denied for function rate_limit_check_and_record`.
- **Fail closed (accept gate):** the welcome accept attempt was refused with the generic "Too many attempts. Try again later." — indistinguishable from a real limit hit, as designed — with the branch log `… accept_invite check failed (failing closed): permission denied for function rate_limit_check_failures`; the accept RPC never ran.
- **Restore + positive control:** grants restored and ACL-verified to the exact migration posture (`authenticated` true ×2, `anon` false ×2), then the identical invite → gate → accept chain re-driven to full success — client linked, membership row created, and a `staff_invite:<uid>` attempt row recorded in `rate_limit_log` (the limiter demonstrably live again). The revoke was the sole cause of the refusal. The positive control also re-exercised the **mint-at-POST chain** end-to-end on staging in passing (gate → POST-mint → GoTrue verify → authed `/welcome` → accept), consistent with the Step-4 closure above.

**Concurrent-session collision, disclosed.** The first positive-control attempt was invalidated mid-run: a sibling dev session re-seeded staging (seed-org teardown + re-create; the Odyssey org was re-born at 10:40:57Z), deleting the first probe client, its token, and its audit rows out from under the in-flight browser flow. Root-caused via the seed org's `created_at` — the "impossible" state (a vanished row with no audit trail) is the signature of an org-level wipe, which removes `audit_log` by design. No damage in either direction: the fault injection was `authenticated`-scoped so the sibling's service-role seed was unaffected, and the re-run positive control above is the recorded one. Operational lessons, recorded: **staging is a shared mutable surface across concurrent sessions — a sibling wipe/reseed can invalidate in-flight browser verification; when staging state looks impossible, check the seed orgs' `created_at` first.** Also caught in the same forensics: §12 Part B's `communications` log rows FK-restrict both client and sender deletion — probe teardown must delete comms rows first. All fixtures torn down clean (final sweep: 0 probe auth users, 0 probe clients).

### 2. Archived-client tripwire — pgTAP `56` extended 8→18, green on staging

The reviewer's second actionable: C-3's dismissal rested on the reading-only universal claim ("every client-readable policy carries the `deleted_at` gate; denial is immediate at archive commit"). Partially pre-empted since the close — pgTAP `56`/`57` landed with CN-7 (2026-07-02), covering the archived clients-row lockout and full-surface cross-tenant isolation — but the **dependent-table** half of the claim, probed from the archived client's own session, was still untested. The extension tests it with the real mechanism, not a seeded-archived row (a direct INSERT of archived state would bypass the `client_cascade_thread_archive` trigger and mis-test the thread surface): `cancel_client` — the fixture client test 6 already archives via the production `soft_delete_client` RPC — is now linked to a real client-role login and seeded with the full portal-readable perimeter (active program, session, message thread, plus its existing future appointment), asserted **before** (tests 9–13: all five surfaces visible — the anti-trivial controls) and **after** (tests 14–18: all five return zero) the archive, inside the same transaction. Zero at commit, not at token expiry — the universal claim is now a regression tripwire. `clinical_notes` is deliberately absent: the client role is blanket-denied there, so archived-gating is moot (noted in the test header; the missing `rls_clinical_notes_select_client_denied` automated assertion remains separately indexed at checklist §8 as an `rls-policies.md` deferral). **Run green 18/18 on staging** (`supabase db query --linked -f`).

### 3. Password-rule census — re-walked, no drift

The census trigger ("any change to password policy — including the C-7 Pro-upgrade re-trigger") fired at the 2026-07-21 Pro cutover and was not discharged then; discharged now. All sixteen in-repo literals still read 12 — welcome action check + error (`src/app/welcome/actions.ts:38-39`), WelcomeForm `minLength` + hint (`:44-45`), invite template HTML + plaintext (`client-invite.ts:80,128`), reset action check + error (`reset-password/actions.ts:38-39`), reset page `minLength` ×2 + hint (`page.tsx:73,77,94`), signup action check + error (`signup/actions.ts:24-25`), SignupForm `minLength` + hint (`:71,75`), and `supabase/config.toml:47` — and the seventeenth touch point, live GoTrue `password_min_length`, is 12 per the Pro-cutover run (checklist §1/§7; `runbooks/verify-auth-config.md` 2026-07-21 run-log row). One annotation since the census: `reset-password/actions.ts:33` now carries a "KNOWN DE-DUPLICATION DEBT" comment naming the coupling — an acknowledgement, not drift. HIBP going live at the cutover changed no literal (a GoTrue-side check, not copy). **No drift; the trigger closes clean.**

### Structural notes, carried forward

**(a) The premortem's blind spot — conceded.** Both production outages in this section (`c301832` poison cookie, `c7750be` missing env var) came from a failure class — deployment/configuration drift — that the F-1..F-14 premortem never modelled, and the closing commit's §4 disposed of every *named* failure mode without conceding the mis-scope. The corrective artifacts (`/api/health`, `staff-login-path-verify.mjs`, `runbooks/deploy-the-app.md`) treat the symptom class; the template fix is recorded here as the standing rule: **every future premortem — any structural rework re-entering the polish protocol — includes a configuration/environment-drift lens alongside the code, UX, and security lenses.** A matching one-line amendment to CLAUDE.md's polish-pass protocol step 3 is proposed to the operator (CLAUDE.md changes are operator-gated; not made unilaterally here).

**(b) C-12's bundled review — acknowledged.** The final gap of a section should not receive its only review inside the section-level pass; future section closes give the last item its own reviewer round before the closing commit is written.
...
**Migration `20260721150000` applied to production at the prod-apply sitting** (`runbooks/prod-apply-sitting-2026-07.md`), which ran 2026-07-23 AEST — **2026-07-22 in UTC, the timezone this appendix records in.** The migrations table carries no apply timestamp, so the exact instant is not separately logged, but it is bounded: the apply fell within **(2026-07-22 21:57:15 UTC, 2026-07-22 22:02:23 UTC)** — after the stack merge commit, before the frontend deploy — and the **frontend deploy went live 2026-07-22 22:02:23 UTC.** The DB-before-frontend ordering the skew-safe design depends on holds by that bound (the nullable column landed before the new frontend served). The mint-at-POST frontend (`src/lib/clients/invite-link.ts`, `src/app/i/[id]/actions.ts`, the send path inserting `action_link: null`) deployed the same sitting; `invite_tokens.action_link` is nullable on prod (types parity zero-diff); the deploy-skew legacy path (stored-link rows redirect directly, only null-link rows mint) shipped as designed.

**Prod behavioural evidence — the mint-at-POST path exercised end to end on production (all times UTC, 2026-07-22).** An operator-owned throwaway (client "Alias Test", token `beacb280-c136-4ac6-8b44-5f27387ab509`) was created 23:45:36 and tapped 23:46:01 — that tap wrote `consumed_at`, claiming the row. On the winning claim the OTP link was minted and **stored** on the row: a live read showed `action_link` present with the GoTrue `/auth/v1/verify` shape and `type=invite` (268 chars) on the production project ref, **not** the `/i` gate-URL shape — proving both that the mint succeeded and that the best-effort store write landed (so this row is *not* the consumed-with-null-link store-failure state). The invitee onboarded through to `/welcome` with `clients.user_id` linked and `onboarded_at` set, and `auth.invite.accepted` was emitted 23:46:21 with the matching `client_id`. The authenticated landing is evidenced by the server-side accept action having run (its `auth.invite.accepted` emission) together with `onboarded_at` being set — the two together prove production GoTrue accepted the minted `redirectTo` and completed the exchange. A second GET of the gate returned the "already used" consumed shell live in prod on the mint-at-POST row itself (C-11 burn intact, not merely on a legacy row). Teardown was atomic, leaf-to-root, post-census zero; the two append-only `auth.invite.*` audit rows were retained by design.

**Two-TTL clarification, so an evidence row is not misread.** Two expiries exist and C-14 moves only one. `invite_tokens.expires_at` — 8 hours from token creation — is the **gate-token** TTL, unchanged by C-14. The **OTP** TTL is the GoTrue verify link's own, and it is the one C-14 delivers: it now starts at the tap rather than at send. An evidence row whose `expires_at` sits 8 hours after send is therefore reporting the **gate token, not the OTP**, and does **not** indicate the OTP-starts-at-tap property is unverified.

**Disclosed edge — mint-success-then-store-failure** (recorded at the same standard as the double-tap-loses-to-bounce disclosure in the section above). If the mint succeeds but the best-effort store write of the minted link fails, `continueInviteAction` still redirects the human in on the **live minted link** (it rides the redirect to their browser exactly as on a successful store), and the row stays `consumed_at` set with `action_link` NULL permanently. This is **UX-degrading, not security-degrading**: no OTP leak (the link only ever reached the tapping human's browser, never a store a third party reads), and no reuse (`consumed_at` stays set, and the un-claim fires only on a *mint* failure, never a *store* failure). The cost is a silent loss of the C-11 double-tap grace window for that one row, recoverable only by a staff resend spawning a fresh token. The resulting consumed-plus-null-link state renders the "already used" shell **gracefully — no throw, no null redirect.**

This appendix **supersedes** the earlier "prod behavioural evidence pending / sign-off paste-back pending" note: both are now discharged.

---

## Sign-off — C-14 deferred item 1 (2026-07-23)
