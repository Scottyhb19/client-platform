...
**Still open — full production onboarding smoke-test (not a gate item; pre-beta confirmation).** This proves the redirect *target* is correct; it does not yet confirm a real client can complete onboarding end-to-end on production (tap → `/auth/callback` → set password at `/welcome` → portal), which to date has only ever completed on localhost. **Re-trigger / gate:** before the first friends-and-family beta tester, run one real invite to a controlled mailbox, tap on production, complete `/welcome`, and confirm the portal loads. The `/login` poison-cookie fix (`c301832`) and this redirect fix are both prerequisites now in place.
