...
- **C.1 — Auto-extend for repeat past block end (closed; pgTAP 10 14/14 green on staging 2026-05-03).**
  - First user-walkthrough caught: repeat picked an end date past the block's end (e.g., source in a 4-week block, end-date in July). Without auto-extend the RPC silently skipped every target outside the block and the user saw fewer copies than expected.
  - **Fix in `repeat_program_day_weekly`:** before the loop, compute the source program's required duration to cover the picked end date (`(p_end_date - src.start_date) / 7 + 1`). If it exceeds the current `duration_weeks`, attempt an `UPDATE programs SET duration_weeks = required_duration`. The EXCLUDE constraint `programs_no_active_overlap` handles the multi-block edge case — if extending would overlap another active block, the UPDATE raises 23P01 and we catch it. The loop then falls back to the original behavior (each target lands in whichever active block covers it; out-of-coverage dates skipped and reported in `no_program_dates`).
