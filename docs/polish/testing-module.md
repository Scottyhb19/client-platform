...
- **D.6 — Visibility model simplification (closed, gated on migration apply + types regen + pgTAP re-run).**
  ...
  - **Type-check clean** post-edits. The user must, in order: `npx supabase db push` (applies the migration), `npm run supabase:types` (regenerates `database.ts` to drop the column), then re-run pgTAP 01, 02, 04, 08 in the SQL Editor. After that, manual UI walkthrough: confirm publish buttons appear on every non-Tampa test card, settings → tests no longer shows the Visibility column, custom-test builder no longer shows the Visibility field.
