...
**Acceptance tests run and results (all on live, 2026-07-02).** pgTAP `56_archived_client_access.sql` **8/8** (staff-reads-archived; archived client''s own login sees zero rows; foreign-org staff zero; live-policy control; anon 42501; archive cancels appointment `cancelled/staff`; reminder cascade `cancelled`; restore round-trip with booking staying cancelled). Regression battery re-run green: 17 (8/8 — §6 rule, RLS touched), 38 (38/38 — RPC grants survived the CREATE OR REPLACE), 46 (3/3), 54 (8/8). `tsc --noEmit` and `next build` green. Render tier accepted at F&F per go-live-checklist §5b; operator scenarios CP-ARC-1..3 are the browser pass.
