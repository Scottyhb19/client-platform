...
- pgTAP, run as BEGIN/ROLLBACK batches against the live project via `supabase db query --linked -f`: tests 09–22 all green — **123 visible assertions**, including the 61 new/extended ones from this pass (10: 20, 11: 12, 21: 15, 22: 14). Tests 01–08/14 predate the `_tap` output pattern and return no assertion rows through this runner; all executed without SQL errors except **test 05**, which fails on a fixture broken by the 2026-05-01 publish-gate schema change (`client_publications.test_id` NOT NULL) — pre-existing since May, testing-module-owned, flagged as a spawned task rather than fixed in this pass. **Test 13 had one stale red assertion** (default day label 'A' vs the 20260504130000 relabel to 'Day 1') — repaired test-side in this pass; it had been failing since 2026-05-04.
