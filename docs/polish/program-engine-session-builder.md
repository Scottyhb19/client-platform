...
- **Go-live checklist:** no new entries — the SECURITY DEFINER grant sweep already indexes the program RPC families; G-1/G-2/G-3's new or replaced functions must keep the REVOKE-from-PUBLIC / in-body-guard pattern that sweep verifies. The G-2 functions (`save_program_as_template`, `create_program_from_template`) and the replaced clone/insert functions all carry it.
...
- `npx tsc --noEmit` clean; `next build` production-clean. The one eslint error in the touched files (`SectionTitleField` set-state-in-effect) is pre-existing on HEAD and load-bearing for superset sibling sync; not addressed.
