...
| **D** | B1 (staff half) | Add a collapsible per-session expander in the staff `clients/[id]/program` view. Each row in the program day list expands to show: client feedback text, session RPE, duration, set count, average RPE. Schema already has the columns; this is a query + UI pass on the staff side. | C (so test data exists) |
