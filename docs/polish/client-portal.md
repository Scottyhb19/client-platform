...
| **J** ✓ | E1 — **closed 2026-05-15.** | Data-tab redesign per session-builder reports panel. Three sub-phases shipped: **J-1** (commit `827ee8a`) extracted comparison helpers (`pointAtSession`, `pickPreviousBefore`, `groupHistoryBySession`) into `src/lib/testing/comparison.ts`. **J-2** (commit `535ee58`) replaced the flat list with session-grouped per-publication hierarchy, added collapsibility (newest expanded, 300ms reveal), compacted MilestoneChart first-capture to a borderless two-line layout, added the staff `TestPublishDialog` "Session battery" select + RLS migration `20260515120000_client_select_test_batteries.sql` so the portal can SELECT battery names via a SECURITY DEFINER helper. **J-3** added the per-card Baseline ↔ Previous toggle + MilestoneChart anchor-swap. **Phase M deferred** to `docs/deferred-prompts.md` — staff Reports Category ↔ Test Battery view toggle, with all decisions (Q-J11/12/13) pre-locked. Gap doc + sign-off log: [`client-portal-data-tab.md`](./client-portal-data-tab.md). | A (uses `.portal-card`); independent of G |
