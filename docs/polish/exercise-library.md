...
- **§12 acceptance gate clause 2 ("the delete warning fires on an in-use exercise") — OBSERVED 2026-06-12 (post-backfill).** Reviewer correctly flagged that the earlier evidence proved the *count rendering* but only *asserted* the safety warning — the pre-push delete round-trip ran when `usage_count` was still 0 (warning structurally couldn't fire), and the post-push check only confirmed cards render "used N×". Re-verified directly against the live DB (throwaway staff user, hard-deleted after): `window.confirm` intercepted to capture the exact string `handleDelete()` passes and return `false` (non-destructive — no soft-delete fires). Two cases observed: **Kickstand Hinge (usage 38)** → `Delete "Kickstand Hinge"? / Used in 38 program days. The exercise will be hidden from the library; existing prescriptions are unaffected.` (plural); **Wall Copenhagen (usage 1)** → `Used in 1 program day.` (singular — the `=== 1 ? 'day' : 'days'` branch). Post-check confirmed both cards still present and count still 28 (cancel was genuinely non-destructive). The gate is now met by observation, not inference.
