...
| **B. Density tightening** | Card padding 16/20px → 12/14px. Demo-video slab 140px-tall → 96×60px thumbnail bottom-right of left column. Set rows 32px → 26px. Extras row gap 12px → 8px. Card target height ≈ 220px (was ~360px). Three exercises fit on a 1080p viewport without scrolling. | A (tokens — so we don't bake new hardcoded values during the tightening). |
