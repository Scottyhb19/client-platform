...
| **C. Per-set storage** ✓ *signed off 2026-05-07* | New `program_exercise_sets` table + RLS + soft-delete RPC + audit register. Update `addExerciseToDayAction` to insert default-count rows on add. Update `client_get_program_day_exercises` RPC. Update SessionBuilder set-table to read/write per-set. Drop `program_exercises.sets/reps/optional_*` columns in a follow-up migration after a few days of stability. Portal `Logger` — verify still works (it already renders per-set). *Landed: migrations 20260507100000 (table + RLS + audit + soft-delete RPC), 20260507100100 (`client_get_program_day_exercises` v2 returning `prescription_sets jsonb` — DROP+CREATE because the return shape changed), 20260507100200 (one-shot backfill: each live program_exercise gets N rows mirroring the legacy scalars; legacy `rpe` folds into `optional_metric='rpe'` / `optional_value=rpe::text` when there's no other optional pair). Q6 sign-off: NO `rpe` column on the new table — prescription RPE goes via `optional_metric/optional_value` once Phase F lands the metric dropdown. Partial unique index on `(pe_id, set_number) WHERE deleted_at IS NULL` (matches project convention; allows stepper to reuse set_numbers post-soft-delete). Three-hop RLS walk via `program_exercises → program_days → programs`. New server actions: `updateProgramExerciseSetAction`, `addProgramExerciseSetAction` (copies last live set's values; `max(set_number)+1`), `removeProgramExerciseSetAction` (RPC). `addExerciseToDayAction` stops writing legacy columns; cleanup-via-soft-delete on per-set fan-out failure. `updateProgramExerciseAction` allowlist trimmed (sets/reps/optional_value/rpe removed). `SessionBuilder` `SetTable` iterates `pe.prescriptionSets[]`; every cell independently editable (the row-1-master / 2..N-static branching is gone). `ExtrasRow` drops RPE (Rest + Tempo only). Portal page reshapes `prescription_sets` jsonb → `LoggerExercise.prescribedSets[]`. Logger `ActiveSet` prefills load/rpe by routing on `optional_metric`. `buildRxLabel` shows the eyebrow summary only when all sets are uniform (Q5=a). Verified via SQL spot-checks (backfill coverage, audit walk, per-set shape) and UI smoke (per-row edit persists, stepper +/- behaves, portal Logger renders per-set prescription).* | B (set rows are at their final height before becoming independently editable). |
| **D. Action bar between + insert-in-place + groups-across-bar** ✓ *signed off 2026-05-07* | Move `CardActions` to a new `BetweenCards` slot. New `addExerciseToDayAction` overload accepting `insertAfterPeId`. New `groupAcrossActionBar` action. *Landed: migration 20260507100300 — new `insert_program_exercise_at(p_day_id, p_exercise_id, p_after_pe_id) RETURNS uuid` SECURITY DEFINER RPC; atomic shift+insert+per-set fan-out under one transaction; three-hop org check inside; group inheritance baked in (anchor and post-shift below share `superset_group_id` ⇒ inherit, otherwise solo). `addExerciseToDayAction` now takes a discriminated-union `slot: InsertSlot` (`{ kind: 'append' } | { kind: 'atStart' } | { kind: 'after'; afterPeId }`), defaulting to append; append path unchanged from Phase C, the other two route via the new RPC. New `groupAcrossActionBarAction(beforePeId, afterPeId)` handles all four Q3 cases (both ungrouped → mint group, boundary → ungrouped joins, adjacent different groups → merge into upper, same group → defensive no-op). `groupWithAboveAction` deleted. `SessionBuilder.tsx`: `CardActions` removed; new `BetweenCardsBar` (1px hairline + small icon-only `BarButton`s — `Plus` always, `Link2` only when grouping the pair has effect) walked into a new `ExerciseList` component that interleaves bars between every adjacent pair plus top + bottom; `insertSlot` lifted to `SessionBuilder`; `LibraryPanel` shows an "Inserting at top / Inserting after {name}" banner with a Cancel `X` while a slot is armed; library-pick consumes the slot then clears it. Ungroup affordance moved off the per-card pill onto the row's icon strip via a `Unlink` `IconButton`, visible only when the card carries a `superset_group_id` — `ungroupFromSupersetAction` unchanged. Active-slot bar visual: filled slate background + white icon, matching the spine token. Spine alignment fix: `SupersetBlock` switched from a flex-stretch-with-distributed-letters layout to a 2-column CSS grid where each B1/B2/B3 letter sits in row `idx*2+1` (the card's row) and the in-group bar + `−` separator sit in row `idx*2+2`; spine background is absolute-positioned across the full block height. End-to-end verified via dev-server logs (`groupAcrossActionBarAction` + `addExerciseToDayAction` with `{kind: "after"}` both POST 200) plus a user-shared screenshot showing the new layout; spine alignment fix landed on top.* | C. |
| **E. Section-title dropdown + dedupe + library filter chips** ✓ *signed off 2026-05-07* | Wire `SectionTitleField` to `section_titles`. Dedupe consecutive same-section bars in render. Add movement-pattern + tag chips to `LibraryPanel`. *Landed: page loader gains three queries (`section_titles`, `movement_patterns`, `exercise_tags`) plus enrichment on the existing exercises query (now also pulls `movement_pattern_id` and a flat `tag_ids` array via `exercise_tag_assignments`). New `addSectionTitleAction(rawName)` server action — RLS-scoped INSERT into `section_titles` with caller's org, length validation 1–60 (matches the table CHECK), `sort_order = max+10` so new sections sort to the bottom; the duplicate-name path (23505) returns a friendly error. `SectionTitleField` rewritten as a two-mode component: `select` mode = native `<select>` with sentinel `(— Section —)` + the org's seeded titles + a fallback `<option>` for legacy free-text values not in the list (preserves existing data) + trailing `+ Add new section…` sentinel; `creating` mode = inline text input, Enter commits, Esc cancels. Commit fires `addSectionTitleAction` and `updateProgramExerciseAction` in parallel — duplicate-name on add is soft-failed because the section already exists in the org's list, so the EP's intent is satisfied either way; `router.refresh()` propagates the new option to other cards' dropdowns. `LibraryPanel` gains two `FilterChipRow` components — multi-select pattern chips above the search, multi-select tag chips below, with charcoal-on-fill when selected. Filter logic = AND across categories, OR within (Q3 sign-off). Reset button surfaces when any filter is active. Q1 (free-text vs FK) confirmed: `section_title` stays free-text on `program_exercises` — dropdown is a UI helper, not a constraint. Q2 (empty-section render): the existing walker already correctly omits the `SectionStrip` when `section_title` is null/empty (`if (section && section !== lastSection)`) — confirmed, no code change. Q4 (audit register for `section_titles`): deferred to Phase I polish round per sign-off. Q5 ("+ Add" UX): native-select sentinel + inline-input toggle, single field of focus. No new tokens, no new migrations.* | None of A/B/C/D strictly. Independent; can run in parallel with D. |
| **F. Swap-in-place + "Load / Notes" metric dropdown** ✓ *signed off 2026-05-08 (integrated)* | New `swapProgramExerciseAction`. New `[value][metric]` cell driven by `exercise_metric_units`. *Landed on `claude/frosty-kalam-5b342a` then cherry-picked into the F+G+H consolidation branch on 2026-05-08; migration filename bumped from `20260507100400_swap_program_exercise.sql` to `20260507100500_*.sql` to dodge G's already-applied 100400 timestamp (live Supabase had G's `reorder_program_exercises` migration applied before F's existed; without the rename `supabase db push` would silently skip F's RPC as already-applied). New `swap_program_exercise(p_pe_id, p_new_exercise_id) RETURNS uuid` SECURITY DEFINER RPC — atomic soft-delete-old + insert-new + per-set fan-out under one transaction; upfront caller-org check on both old `program_exercise` and replacement `exercise` so EP gets a clean "not found in your organization" error before the BEFORE-INSERT trigger fires; preserves slot fields (`sort_order`, `section_title`, `superset_group_id`); resets prescription to the new exercise's defaults. History preservation: `exercise_logs.exercise_id` is `NOT NULL REFERENCES exercises(id)` so old logs keep their FK after the swap. New `swapProgramExerciseAction(programExerciseId, newExerciseId)` server action wraps the RPC. UI: clicking an exercise name arms `swapTarget`; right panel forces tab to library + shows "Replacing: {old name}" banner with Cancel; next library-pick consumes the slot. Load/Notes column rewritten as `[value][metric]` — value input + metric `<select>` driven by `exercise_metric_units` (filtered `is_active = true`, `deleted_at IS NULL`, ordered by `sort_order`). New `updateProgramExerciseMetricAction` server action.* | C (per-set storage means swap re-creates per-set rows from new exercise's defaults). |
| **G. Drag-and-drop reorder** ✓ *signed off 2026-05-07* | `@dnd-kit/core` + `@dnd-kit/sortable` + `@dnd-kit/utilities` install. Grip handle becomes a real drag handle; up/down arrows kept as keyboard a11y fallback. New `reorderProgramExercisesAction(dayId, orderedIds, movedPeId)`. Drop-into-superset / drop-out-of-superset / cross-group / within-group edge cases. *Landed: migration 20260507100400 — `reorder_program_exercises(p_day_id, p_ordered_ids[], p_moved_pe_id)` SECURITY DEFINER RPC, single-hop org check via pd.program_id. Phase 1 rewrites every row's sort_order via UPDATE FROM unnest(... WITH ORDINALITY) (no UNIQUE on (day_id, sort_order) so single statement is safe — sentinel-swap not needed). Phase 2 applies the insert rule to the moved card only — both new neighbours share a non-NULL group ⇒ join; otherwise solo — mirroring `insert_program_exercise_at` (Q3 sign-off 2026-05-07: server re-derives from position). Defence: skip group re-derivation when the moved card's sort_order didn't actually change, otherwise a non-moving card with only one same-group neighbour would be set solo. Phase 3 runs singleton cleanup over any group reduced to 1 member, mirroring `ungroupFromSupersetAction`'s tail. New `reorderProgramExercisesAction` server action wraps the RPC. `moveProgramExerciseAction` rewritten internally to delegate to the same RPC (read live ids, swap with neighbour client-side, hand to RPC) so the arrow buttons inherit group rederivation + singleton cleanup — pre-rewrite a bare sort_order swap let the arrow split a superset into a non-contiguous run, which the walker can't render (duplicate React key crash). DnD wiring via `@dnd-kit`: `DndContext` + `SortableContext` (`verticalListSortingStrategy`) wrap the left column's `ExerciseList`. Three sensors — `PointerSensor` (5px activation distance to allow grip-clicks without instant drag), `TouchSensor` (200ms delay + 5px tolerance), `KeyboardSensor` (`sortableKeyboardCoordinates`). New `SortableCardShell` wraps the white card div used by both Solo and Superset members; `DragHandleContext` bridges its `useSortable` lifecycle to the `DragHandle` button deep inside `ExerciseBody` (replaces the decorative `<GripVertical>`); `touchAction: 'none'` on the handle is load-bearing so the TouchSensor wins over the browser's scroll gesture. `DragOverlay` renders a sparse name-only ghost (full card body would re-render on every pointer-move and look noisy). `id="session-builder-dnd"` on `DndContext` makes `aria-describedby="DndDescribedBy-..."` deterministic across SSR + client hydration (without it, @dnd-kit's internal counter starts at different points on each pass — fired a hydration mismatch on every drag handle). Walker key changed to `grp-${groupId}-${i}` so legacy non-contiguous data renders as two visual blocks rather than crashing; the next move on such data normalises it via the RPC. Verified end-to-end: solo→solo reorder, solo→middle of superset (joins), member out of 3-superset (others stay), member out of 2-superset (singleton cleanup dissolves), cross-group drag (leaves A, joins B; A's lone survivor cleaned up), within-group shuffle, arrow buttons still work, 8 test cases passing.* | D (between-card slot model is the drop-target shape). |
| **H. "Last logged" footer** ✓ *signed off 2026-05-08* | New per-card data fetch on the page loader. Render at bottom of prescription column. *Landed: page loader gains an `exercise_logs` query joined `sessions!inner` filtered on `sessions.client_id = $clientId` + `exercise_id IN (page's exercise_ids)` + `deleted_at IS NULL` + `completed_at IS NOT NULL`, ordered `completed_at DESC`, embedded `set_logs(set_number, weight_value, weight_metric, reps_performed, deleted_at)`. No DISTINCT ON in supabase-js — TS dedupes by `exercise_id` taking the first hit (most-recent), filters `set_logs.deleted_at IS NULL`, sorts by `set_number`, builds a `Map<exercise_id, LastLogged>`. Each `ProgramExercise` carries a new `lastLogged: LastLogged \| null` field; `LastLoggedSet` keeps the raw weight/metric/reps so the renderer (not the loader) decides uniform-vs-range. New `LastLoggedFooter` subcomponent in `SessionBuilder.tsx`, rendered as the last child of the right (prescription) column under `ExtrasRow` — `marginTop: 'auto'` pins to the bottom regardless of set-table height; 1px hairline divider above (matches prototype's `rx-last`). `formatLastLoggedSummary` builds the body text per Q1a sign-off: reps uniform → single value, varying → `min-max`; `weight_metric === 'bodyweight'` → drop `@`, append `BW` (`3 × 8 BW`); has weight → `${value\|range} ${metric}` (`4 × 6 @ 75-80 kg`); no reps → fallback `N sets`; mixed metrics across sets → set 1's metric wins (defensive). Date format Q2a: relative — reuses `timeAgo()` from `_components/reports/helpers.ts` (`9 days ago`). Slot location Q3a: bottom of right column under `ExtrasRow` (data-adjacent-to-data). Component returns `null` when `lastLogged === null`, so pre-launch (zero rows) the line never renders. No DB migration, no RLS change, no new tokens. Performance note retained: pre-launch trivial; post-launch monitor and switch to a SECURITY DEFINER RPC if profiling shows hotspot. Verified via type-check (clean); browser verification deferred — pre-launch the footer renders nothing for any card by design (the `pe.lastLogged === null → return null` short-circuit), so the only observable behaviour pre-launch is "no regression on existing builder render," covered by the type-check.* | C (set storage shape symmetry with `set_logs`). |
