...
- **Constant-time EF bearer compare — in repo, awaiting the next `auth-events-alerts` EF redeploy to reach prod** (non-blocking, non-exploitable).
...
**Remaining operator step (does not gate this sign-off):** redeploy the `auth-events-alerts` EF to prod to carry the constant-time bearer compare (and the comment-only cadence-doc changes) — `supabase functions deploy auth-events-alerts --project-ref azjllcsffixswiigjqhj`.
