...
| 3 | Supabase creates `auth.users` row, sends verification email | `enable_confirmations = true` in `supabase/config.toml:52`; signup action handles the `data.session === null` branch by returning a check-email state | ✓ Conformant from code. Email actually arriving requires the Supabase dashboard SMTP / email-template config — not verifiable from code. | `supabase/config.toml:52`, `src/app/signup/actions.ts:43-47` |
...
| 12.2 Email verification ON for owner | EP owner cannot log in before clicking confirmation link | `enable_confirmations = true` in `supabase/config.toml:52`; signup action's `data.session === null` branch returns a check-email state. Application does NOT independently verify `user.email_confirmed_at` — it trusts the Supabase toggle. If the toggle is flipped off in dashboard, the application silently accepts unverified sessions. | ⚠ Conformant via Supabase only; no application-layer defence-in-depth → **G-7** |
| 12.3 Session duration: 30 days uniform | Refresh-token lifetime = 30 days for all roles **[Target amended 2026-07-22: no absolute refresh-token-lifetime control exists in Supabase; the target is restated as `sessions_timebox = 720 h` + rotation on + reuse interval 10 s, satisfied 2026-07-21 — see the G-4 restatement in "Reviewer revisions — 2026-07-22". A future audit re-deriving this row must measure against the restated target, not this cell's original wording.]** | `config.toml` sets only `jwt_expiry = 3600` (access token), `enable_refresh_token_rotation = true`, `refresh_token_reuse_interval = 10`. Refresh-token lifetime is not explicitly configured in code; the Supabase default applies (configurable in dashboard, **not verifiable from code**). The `/login` form's "Keep me signed in for 30 days" checkbox is decorative — the `remember` form field is not read by the action, and the session lifetime is whatever Supabase enforces regardless of the checkbox state. | ⚠ Cannot verify 30-day target from code → **G-4** and **G-8** |
...
| **F-12** | **`'Pending'` placeholder coupling between trigger and bootstrap RPC.** A change to `handle_new_auth_user()` (e.g. changing placeholder strings or removing them) silently breaks `create_organization_with_owner`'s name-update — first/last name stay 'Pending'. No test catches this. | Low × Medium | **G-13 — CLOSED 2026-05-25** |
...
**G-10 — Resend confirmation email UI for staff signup — BUILT.** Closes F-5.
Previously, if the EP's confirmation email was lost or expired, the only paths were (a) start a new signup with the same email, (b) operator intervention. A resend-confirmation control now appears on the signup check-email view and on the `/login` error path when the error is "Email not confirmed", via a shared server action calling `auth.resend({ type: 'signup', email })`. The email is held in React state and never placed in the URL. See the Track B closure record below and B.6 for the mechanism.
...
`handle_new_auth_user()` inserts `('Pending','Pending')`; `create_organization_with_owner` UPDATEs `user_profiles SET first_name=..., last_name=... WHERE user_id = caller_id AND (first_name = 'Pending' OR last_name = 'Pending')`. If a future change to the trigger alters the placeholder strings, the UPDATE silently does nothing and every new staff user's profile is left at the new placeholder. No test guards this. Closing this: either (a) remove the WHERE filter on `'Pending'` and trust the SECURITY DEFINER context (the RPC is only called from `/onboarding/org`, and the pre-flight `IF EXISTS (...) RAISE EXCEPTION` already guards against double-updates from re-onboarding); (b) reference a constant in both places via a Postgres `CREATE FUNCTION pending_name() RETURNS text` or similar; (c) add a pgTAP test asserting the round-trip. Option (a) is the cleanest. **Recommendation** (no UX-visible impact today; this is robustness against a future code change).

**CLOSED 2026-05-25.** Resolved via option (a) — removed the redundant `first_name`/`last_name = 'Pending'` predicate from the UPDATE's WHERE clause, keeping the load-bearing `user_id = caller_id` — plus option (c), a pgTAP round-trip test (`supabase/tests/database/15_g13_pending_round_trip.sql`) proving `'Pending'` before then real names after, run green against production. Option (b) was not taken. The migration (`20260524202523_bootstrap_drop_pending_filter`) is in commit `014f18a`; the pgTAP test and this closure note land together in the following G-13-closing commit. Implementation note: the migration was built as `DROP FUNCTION` then plain `CREATE` (signature unchanged), so it also restates the `COMMENT`, the `REVOKE EXECUTE FROM PUBLIC`, and the `GRANT EXECUTE TO authenticated` to preserve the function's prior access posture — a bare `CREATE` would have left it at the Postgres execute-to-PUBLIC default. This grants-restoration was a regression caught at the design gate, not part of the original a/b/c framing above.
...
- **G-13** (`'Pending'` placeholder coupling cleanup) — P2 recommendation, approved. **CLOSED 2026-05-25** — see B.8 closure note below.
...
#### B.6 — G-10 (P2 recommendation): Resend-confirmation UI — BUILT

**Both dependencies resolved during the build:** `@supabase/supabase-js` 2.103.3 supports `supabase.auth.resend({ type: 'signup', email })` on the normal cookie SSR client, no service-role required. The resend rate-limit is NOT the 30/hour signup limit — resend is governed by Supabase's email-send cap, which on the default mailer is tight (browser testing produced "email rate limit exceeded" after a few sends in one window); see `docs/auth.md §7.1`.
...
**CLOSED 2026-05-25.** The migration (`20260524202523_bootstrap_drop_pending_filter`) shipped in commit `014f18a`; the pgTAP test `15_g13_pending_round_trip.sql` and this closure note land together in the following G-13-closing commit, the test having been run green against production. What actually shipped, correcting the plan above: the migration is `DROP FUNCTION` then plain `CREATE` (not `CREATE OR REPLACE` — line 429's wording was internally contradictory; plain `CREATE` is what was used). Because `DROP` destroys the function's grants and comment, the migration also restates `COMMENT`, `REVOKE EXECUTE FROM PUBLIC`, and `GRANT EXECUTE TO authenticated`. The types regen (line 436) was run and was a no-op as expected (a function-body change moves no table shapes). No code changed in `src/`.
...
2. **Assert:** `data.session === null` (confirmations on). A non-null session means confirmations are off → alarm. This matches the application's own branch at `src/app/signup/actions.ts:43-47`, but runs as an explicit cadence-able probe rather than relying on a real signup happening.
...
- **G-7 (email confirmations enabled): GREEN — behaviourally verified.** Toggle set in the Supabase dashboard; confirmation-gating exercised end-to-end on 2026-05-26 — an unconfirmed signup was refused at login, then admitted after the emailed confirmation link was followed. Promoted from "toggle set, manual confirmation pending" to behaviourally verified. **Closes G-7.**

- **G-3 (HIBP leaked-password protection): DEFERRED-WITH-TRIGGER — Pro-gated.** The "Prevent use of leaked passwords (HIBP)" setting is locked behind the Supabase Pro tier — confirmed in-dashboard on 2026-05-27, the control is greyed out with an upgrade prompt and cannot be enabled on the free tier. Correction to the record: the 2026-05-21 note above states this toggle was set ON; for G-3 that is not currently the case — the setting is not settable on the present tier, whatever its state was understood to be at the time. Trigger: the Pro upgrade, itself gated on "before any real client data" — same trigger family as G-4. Note: the automated front-door-probe design for G-3 (Track A design table and the G-3 probe subsection) pre-dates this reclassification and is now stale; staleness is tracked for separate cleanup, not resolved here.

**Net Track A:** G-1 closed (verified green, automated). G-7 closed (behaviourally verified end-to-end 2026-05-26). G-3 deferred-with-trigger (HIBP is Pro-gated — confirmed locked in-dashboard 2026-05-27; trigger is the Pro upgrade, same family as G-4). G-4 deferred (Pro-gated). Script, runbooks, and inert org in place. R-4 (manual cross-tenant procedure, `verify-cross-tenant-isolation.md`) is built but not yet run — a separate step, not part of this dashboard-config closure. Stale Track A artifact noted: the automated-probe design for G-3/G-7 (design table rows and probe subsections) was superseded when those gaps' closure paths changed; consolidation tracked separately, not part of this reclassification.
...
- **G-10 (P2 resend-confirmation UI): GREEN — behaviourally verified.** The signup success path no longer redirects to `/signup?info=check-email`; `actions.ts` returns a stateful check-email status that the mounted `SignupForm.tsx` renders as the check-email view, holding the email in React state. A shared `src/lib/auth/resend-confirmation.ts` server action (cookie client, no service-role) and `src/components/auth/ResendConfirmationButton.tsx` provide the resend control on both the signup check-email view and the `/login` error path when the error is "Email not confirmed". The old static check-email branch and its `info` param read were retired from `page.tsx`. The email is never placed in a URL. Acceptance tested: signup done-view renders in place with no flicker and the resend button sends; address bar stays `/signup` throughout and a refresh returns to the empty form (the email lives in state, not the URL, by design); login "Email not confirmed" surfaces the resend control with the exact string matched; the old `?info=check-email` URL is now inert and the `?error=` pass-through still renders. Honesty note: the resend rate-limit is an external Supabase email-send cap, not the signup limit; the default mailer cap is tight and was observed (not tuned) during testing — dashboard tuning is a future check, not part of this closure. **Closes G-10.**

- **G-8 (decorative remember-me checkbox): CLOSED — no-behaviour-change cleanup, code-verified.** The `/login` "Keep me signed in for 30 days" checkbox was decorative — its value was POSTed in FormData and discarded server-side, never read by the login action. Removed and replaced with a quiet caption beneath the password field stating the actual fixed session behaviour. Honest status: there was no behaviour to verify because the control never had any; this is a truthfulness fix to the UI, not a functional change. Commit `960d8fc`. **Closes part of F-3.**

- **G-9 (`/signup` operational gating): CLOSED — behaviourally verified.** Public signup is now gated behind a fail-closed `PUBLIC_SIGNUP_ENABLED` flag (new `isPublicSignupEnabled()` in `src/lib/env/signup.ts`, read at call time, true only for the exact string "true"; missing, empty, or any other value keeps signup disabled). Deliberately fail-closed rather than fail-loud — unlike `site-url.ts`'s unconditional throw — on the reasoning that a missing security gate should keep the door shut, not crash. The signup action guards as its first statement and bounces a disabled attempt to `/signup?closed=1`; the page renders a quiet "Signup is currently closed" state, not a red error. Acceptance tested in browser: closed state renders quietly; direct POST is bounced to the closed state; the error path still works when signup is enabled; a real signup completes end-to-end. Commit `9a19bca`. **Closes F-11.**

- **G-14 (stateful auth forms): CLOSED as form-conversion — but its deep-link preservation claim was later found defective and fixed in a separate security pass.** The form portions of `/login` and `/signup` were converted from stateless server forms into `'use client'` components holding the typed email in React state via `useActionState`, so a failed attempt preserves the email instead of discarding it. The outer pages stay Server Components: login still reads and displays `?error=` from the URL, signup still runs the server-only `isPublicSignupEnabled` gate and renders the check-email panel server-side. Scope was `/login` and `/signup` only; the five sibling auth pages untouched. Commit `9e80c55`, 2026-05-24, browser-tested per its message. Correction to the record: that commit's message claims "the next deep-link return works" — this was subsequently found to be false. A path-conditional middleware bug meant the next deep-link did not survive login for all protected routes, and chasing it surfaced an open-redirect at `auth/callback` (fixed `a81c1ca`) plus two further account-takeover-class defects in the same auth surface. The deep-link is therefore NOT closed by this commit; its correct behaviour depends on the later security pass. That security work is shipped in commits `a81c1ca`, `a7cc273`, `1152df8`, `c705aeb` and documented in the Track C security-pass section below. This bullet closes G-14's form-conversion scope only; it does not assert the deep-link works. Note: this closure resolves the prior internal inconsistency where G-10 was marked BUILT while its prerequisite G-14 carried no status. **Closes G-14 (form-conversion scope).**

**Net Track B:** G-2 closed (behaviourally verified). G-5 closed (code-verified, runtime-unexercised at commit; carried and then-removed silent origin fallback recorded above, with G-11's de-dup-debt characterisation corrected). G-11 closed (typecheck-green, runtime-unexercised; uniform fail-loud origin handling across signup and reset). G-10 closed (behaviourally verified; resend rate-limit is an external email-send cap, observed not tuned). G-8 closed (no-behaviour-change UI truthfulness fix, code-verified, 960d8fc). G-9 closed (behaviourally verified; fail-closed signup gate, 9a19bca). G-14 closed for form-conversion scope only (browser-tested 9e80c55; its next deep-link claim was later found defective and fixed in the security pass at a81c1ca/a7cc273/1152df8/c705aeb, which is shipped and documented in Track C below). All four originally-sequenced gaps committed and pushed (c2a7111, a045e53, fe1aa3b, 164d4aa); G-8, G-9, G-14 add 960d8fc, 9a19bca, 9e80c55. Three origin-construction idioms still exist codebase-wide (env-chain now fail-loud, a header-derived chain in clients/new, and a booking placeholder-chain) — consolidation tracked separately, not part of this closure.

## Track C — security pass: outcome (2026-05-26/27)

Not a planned track. While double-checking G-14's `next` deep-link, the link was found not to survive login — a path-conditional middleware bug. Chasing that surfaced three account-takeover-class defects in the auth surface, none of them in the tracked gaps, all in the code the tracked gaps ran through. The recurring lesson of this section, stated plainly: the dangerous defects were not in the gap list but in the shared machinery beneath it; "felt verified" was the failure mode, exercised-not-assumed the standard. All four fixes shipped and pushed; depth lives in each commit and its touched files — this section is the index.

- **Open redirect at `auth/callback` (every flow: login, signup, recovery, magic-link, OAuth): CLOSED.** `next` was honoured unvalidated through the highest-trust link in the app (the password-reset email). New `src/lib/auth/safe-next.ts` validates `next` as a local same-origin path (prefix/CR-LF string checks plus a `new URL()` origin round-trip against `getPublicOrigin()`), wired at the source and all three callback sinks (idempotent, so each redirect line is safe read in isolation); the misleading pre-existing local `safeNext` was renamed `nextJsLiteral`. Honesty note: the initial diagnosis under-counted the call sites — a sibling header-derived origin at `clients/new` (invite `redirectTo` and `gateUrl`) was caught only at implementation and folded into the same fix, replacing `host`/`x-forwarded-proto` trust with `getPublicOrigin()`. Commit `a81c1ca`.

- **Login-CSRF at `/auth/set-session`: CLOSED.** A cross-origin `mode:'no-cors'` POST of attacker-owned (real, not forged) tokens would land a `Set-Cookie` in the victim's jar, signing them into the attacker's account — the victim then feeds clinical data into it. Now rejects any POST whose `Origin` is absent or not exactly the env-anchored canonical origin (same canonicalisation as `safe-next.ts`), 403 before body-read or `setSession`. Honesty note: this fix also corrected a false in-code comment that claimed `setSession` locally verifies the JWT with Supabase's keys — in `@supabase/auth-js` v2 it does not; verification defers to the next `getUser()`. The same-origin guard, not local token validation, is the control here — the corrected comment matters because the old one implied a protection that did not exist. Commit `a7cc273`.
...
## G-6 partial build — structured auth-event audit log (2026-07-21; staging-only, pending prod apply)

> **Retitled 2026-07-22 per reviewer return** (was "G-6 closure") — a gap that exists in production is not closed by a migration that has only run on staging, and 8/10 events + 0/2 alert thresholds is a partial F-10 closure, not a closure. See "Reviewer revisions — 2026-07-22" below. Closure is recorded here only after the prod apply. Relation to the 2026-05-27 Sign-off: that entry's "G-6 deferred, trigger: before any paying clinical client" line remains the accurate historical record; this section is the current state that amends it — the trigger was consciously pre-empted by the operator-ratified 2026-07-21 pull-forward, and the deferral converts to "partially built, staging-only" as of that date. Neither entry is rewritten; this note is the reconciliation.
>
> **Update 2026-07-23:** both conditions this banner named have since been met — the prod apply ran at the deploy sitting (see "G-6 prod-apply appendix" below; the apply itself fell on 2026-07-22 in UTC), and the sign-off is recorded at "Sign-off — G-6" at the foot of this G-6 run, with its deferred register. The heading's "(2026-07-21; staging-only, pending prod apply)" and the Status line beneath are the frozen at-the-time record, not current state.

**Status: BUILT on staging (pending prod apply at the next deploy sitting + the sign-off ritual).** Pulled forward from the paying-client gate as Step 3 of the 2026-07-21 internal work sequence (operator-ratified). **Partially** closes F-10 to the degree below.
...
## Reviewer revisions — 2026-07-22 (G-6 review: Returned for revision)

**Date:** 2026-07-22. **Reviewer:** claude.ai project chat (challenger role). **Decision on the G-6 closure section as submitted: Returned for revision** — three blockers, four cheap fixes, all evidentiary. This section records the disposition of each against the repo as it stands today. The load-bearing finding of the pass: **most of what the reviewer flagged as missing had already happened between 2026-06-07 and 2026-07-21 and was recorded in the runbooks and the go-live checklist but never propagated back into this doc.** One blocker survives contact with the evidence as a real, still-live defect (the deep-link — reopened below as G-15); one stands as a labelling correction (G-6, retitled above); the rest resolve with citations.

### Blocker 1 — G-6 overstated: ACCEPTED, retitled

The section heading is retitled "G-6 partial build (staging-only, pending prod apply)" with a reconciliation note against the 2026-05-27 Sign-off (see the retitle banner above — neither record is rewritten; the Sign-off's deferral is history, the pull-forward is the amendment). Closure will be recorded only after the prod apply at the next deploy sitting, and even then as **partial** — 8/10 events wired, `jwt.hook_failure` + `cross_tenant_access_attempt` honestly out (no emission point exists), 0/2 alert thresholds (no alerting pipeline exists; rides the observability build-out). The go-live checklist §8 G-6 line already carries the same "BUILT (staging), pending prod apply" wording and needs no change. *(2026-07-23: that last sentence describes the 2026-07-22 state and is superseded — the checklist §8 G-6 entry was flipped to PROD-APPLIED at the deploy sitting and updated again with the sign-off; see "Sign-off — G-6" below.)*

### Blocker 2 — the originating deep-link defect: CONFIRMED STILL LIVE — reopened as G-15

The reviewer asked for the commit and the browser test, or a reopen. The evidence supports the reopen — **there is no commit; the defect is still in the code:**

- The only code path that sets `next` on a login redirect is the middleware (`src/lib/supabase/middleware.ts:170-173`), and its `isProtected` list covers exactly three prefixes: `/dashboard`, `/portal`, `/onboarding` (`:165-168`). That list is unchanged since Phase B1 — `git log` on the file shows only two commits ever (`4393651` Phase B1, `c301832` the 2026-06-10 poison-cookie sanitizer).
- `requireRole` — which guards every staff route *outside* `/dashboard` (`/clients`, `/settings`, `/schedule`, `/library`, `/messages`, via the `(staff)` layout) — redirects unauthenticated users to bare `/login` with **no** `next` param (`src/lib/auth/require-role.ts:34` and `:73`).
- Consequence, unchanged since G-14 shipped: a logged-out deep link to `/dashboard/...`, `/portal/...`, or `/onboarding/...` survives login; a logged-out deep link to `/clients/<id>` (or any other staff route) silently lands on `/dashboard` after login. This is precisely the "path-conditional" behaviour the G-14 correction described.
- None of the four Track C commits touches this: `a81c1ca` added `safe-next.ts` **validation** of `next` where it is already present (callback sinks + login action) plus the clients/new origin fix; `a7cc273` is the set-session CSRF guard; `1152df8`/`c705aeb` are the recovery-ticket gates. Track C made the `next` value *safe to honour*; it never made `next` *get set* for staff-layout-protected routes.

**Correction to the G-14 correction:** the Net Track B bullet's claim that the deep-link's "correct behaviour depends on the later security pass" is itself wrong and is corrected here per the doc's corrections-as-new-notes convention. The security pass is a prerequisite of a future fix (any fix must route through `safeNext`), not the fix. The deep-link defect was never closed by any commit.

**G-15 (new gap, P2) — login redirect drops the deep link for staff routes outside `/dashboard`.** Severity call: P2, not P1 — at f&f scale the staff surface is the operator plus one trusted EP, the loss is a papercut (land on `/dashboard`, re-navigate), and there is no security dimension (`safeNext` already caps what `next` can be). Candidate fix shapes, for approval before any code per protocol step 5 — **reframed 2026-07-22 after the reviewer challenged the original "one-line class of change" wording; the interaction analysis below was verified against the code, not asserted:**

- **(a) recommended — widen the middleware's `isProtected` list to the staff route prefixes** (`/analytics`, `/clients`, `/contacts`, `/library`, `/messages`, `/schedule`, `/settings` — seven additions, enumerated from `src/app/(staff)/`). Honest weight, verified:
  - **The poison-cookie-sanitizer concern checked and ruled out:** the proxy matcher already runs `updateSession` — sanitizer included — on **every** non-static route (`src/proxy.ts:19`); `isProtected` gates only the redirect, not the sanitizer. Widening the list changes nothing about which routes the sanitizer touches.
  - **The second-redirect-authority concern is real and is the actual behaviour change:** for the seven added prefixes, the unauthenticated redirect moves from `requireRole` (fires during server-component render, bare `/login`) to the middleware (fires pre-render, `/login?next=<path>`, purge-cookie carry on the redirect — the exact behaviour `/dashboard` already has). The two authorities agree on destination; the deltas are the added `next`, the earlier fire, and skipped layout work. `requireRole` remains the **sole** authority for the authenticated-but-claimless (`/onboarding/org`) and wrong-role (`/unauthorized`) branches — the middleware checks only user presence and cannot collide with either.
  - **The residual cost is a maintenance coupling:** any future top-level staff route must join the prefix list or its deep-link silently drops again — the same silent-drift class this gap is about. Mitigation to ship with the fix: a pointer comment in `src/app/(staff)/layout.tsx` beside `requireRole`, naming the middleware list as the place a new staff route registers.
- **(b) rejected** — have `requireRole` append `?next=` itself: needs the request path threaded into a helper that deliberately doesn't receive it.
- **(c) surfaced, not recommended now** — invert to protect-by-default with a public-route allowlist (`/`, `/login`, `/signup`, `/forgot-password`, `/auth/*`, `/welcome`, `/i/*`, health/manifest assets): eliminates the maintenance coupling permanently but is a materially larger behaviour change with its own failure mode (accidentally gating a public surface). Right shape if the prefix list ever bites; wrong first move.

Closing G-15 requires the browser test G-14's commit claimed and never had: logged-out deep link to a `/clients/<id>` URL → login → land on that URL — plus the negative control (claimless user still reaches `/onboarding/org`).

### Blocker 3 — R-4 compensation "never run": RESOLVED — it ran twice; this doc was stale

The Sign-off's R-4 line ("built but has not yet been run") was true on 2026-05-27 and went stale eleven days later. The record, all external to this doc until now:

- **2026-06-07 — the manual procedure ran, all checks pass** (commit `3cc98c3`; dated entry in the `runbooks/verify-cross-tenant-isolation.md` run log). Two ephemeral orgs via the real front door, read isolation across **all eight core tenant tables**, write probes denied (`UPDATE` 0 rows; `INSERT` raised `42501`), teardown verified clean by post-COMMIT census.
- **2026-06-07 — the automated pgTAP test landed the same day** (commit `208b09a`, `supabase/tests/database/17_cross_tenant_isolation.sql`, self-fixturing, 8/8 green) — **R-4 itself is CLOSED**, and the manual procedure is downgraded to quarterly belt-and-suspenders per the runbook's Retirement section.
- **2026-07-02 — automated re-run green 8/8** after the API-key migration (commit `be3fc42`; run-log entry).

So the deferral the reviewer called uncompensated was compensated, and then the deferred item itself closed — the boundary claim is no longer "one hook probe"; it is hook probe + eight-table manual pass + a per-migration automated tripwire. **The Sign-off's stated R-4 trigger ("a second practitioner account") was overtaken by events**, exactly as the reviewer's staging observation suggested it would be: the automated test proved self-fixturing against the live project via `BEGIN…ROLLBACK` and never needed the second practitioner. CLAUDE.md's Beta-entry hardening gate carries the same record.

### Cheap fixes — dispositions

- **G-11 throw path unexercised:** exercised — by production, harder than any test would have. On **2026-06-10** the exact predicted failure fired live (owner/staff sign-in 500'd because `NEXT_PUBLIC_SITE_URL` was absent in the prod env — incident #2, recorded in `auth-onboarding-client.md` and go-live checklist §3). The env var is confirmed set in production, `/api/health` now reports this failure class continuously, `runbooks/deploy-the-app.md` makes the health check a standing post-deploy step, and `scripts/staff-login-path-verify.mjs` exists specifically to walk a real staff session through the `safeNext('/dashboard')` call that crashed. The reviewer's instinct ("disproportionate downside if wrong") was correct — it *was* wrong, it *did* fire, and the loop that closed it is on record. Go-live checklist §3 gate: all three items CLOSED.
- **F-2's acceptance contingent on G-9's flag:** the coupling was real for the window 2026-05-27 → 2026-07-21 and is recorded here as the reviewer framed it: during that window, "operator controls the small known account set" held only while `PUBLIC_SIGNUP_ENABLED` stayed false. It is now **moot**: at the 2026-07-21 Pro cutover the operator enabled HIBP and the front-door probe went **GREEN for the first time** (breached password rejected with `pwned`; `runbooks/verify-auth-config.md` run log, 2026-07-21 row) — **G-3 is CLOSED**, so F-2's mitigation no longer rests on the signup gate at all. G-3u (HIBP on `updateUser`, the client welcome/reset path) went green the same run.
- **Sign-off deferred-items ledger, updated by the same evidence:** **G-3 CLOSED 2026-07-21** (above). **G-4 CLOSED 2026-07-21 — satisfied in intent by a different control than the gap named, stated plainly per the reviewer's correction (2026-07-22):** the gap targeted "refresh-token lifetime = 2592000 s". What the operator actually set is **`sessions_timebox` = 720 h** — the absolute session time-box, a different Supabase setting. No configurable absolute refresh-token-lifetime control exists on the current dashboard (the A.1-era "Refresh Token Expiry" field was sought at the Pro cutover and does not exist; the run-log row records the discovery — its "renamed control" phrasing is superseded by this note: not renamed, *different control, only one available*). The time-box satisfies §12.3's intent ("uniform 30-day session") **at least as strongly** as the named target would have: rotation (single-use refresh tokens, behaviourally verified on live 2026-07-02) already kills stolen-token replay, and the time-box caps even an actively-refreshed session at 30 days absolute — which a refresh-token idle-expiry alone would not (an attacker refreshing keeps an idle-expiry token alive indefinitely). F-3's threat is therefore bounded harder than the original design's mechanism would have bounded it. G-4's target is restated as: `sessions_timebox = 720 h`, rotation on, reuse interval 10 s. **R-4 CLOSED 2026-06-07** (Blocker 3). **G-6** converts from deferred to partial-built-staging (Blocker 1). Every deferred item from the 2026-05-27 Sign-off now has a terminal or current-state disposition.
- **Management API channel — conscious reversal of A.1(revised) Q4, recorded (2026-07-22):** Q4 declined *minting a Management API token* to verify the lowest-stakes property. What subsequently happened (runbook note, 2026-06-10) is narrower but is still a posture reversal: the **already-present Supabase CLI access token** (Windows Credential Manager, generic credential `Supabase CLI:supabase`) was reused as a read channel to `GET /v1/projects/<ref>/config/auth` — no new credential was created, but the "no Management API path" stance did not survive contact with the G-7 probe-domain blocker, and the reversal had not been recorded against Q4 until now. The channel is read-only by declared posture (PATCH only to restore a documented target, logged in the run log). The CLI access token itself was **not catalogued in `docs/secrets-inventory.md`** despite authorising config reads *and writes* on both projects — a genuine inventory hole, closed this pass (entry added).
- **Test-target contradiction (May "no non-prod target" vs July "staging"):** both statements were true when written. The May claim described the world before `odyssey-staging` existed; the staging project landed **2026-07-21** with the environment-separation flip (go-live checklist §5, CLOSED). The standing liability the May note recorded (pgTAP against live prod via `BEGIN…ROLLBACK`) is discharged for new work — the 62-file suite now runs against staging by default (`scripts/run-pgtap-staging.sh`), with prod runs an explicit opt-in channel. The R-4-trigger consequence the reviewer drew from this is confirmed and already realised (Blocker 3).
- **"Tracked separately" items get their named home:** (a) the **stale G-3 automated-probe design** flagged in the Track A outcome resolved itself by events — the probe was never discarded; it stayed built into `scripts/verify-auth-config.mjs`, self-armed, and delivered the first-ever behavioural G-3 GREEN at the 2026-07-21 Pro-cutover run. The staleness note is void; no cleanup remains. (b) the **origin-idiom consolidation**: of the three idioms named in Net Track B, the env-chain is fail-loud (G-11) and the clients/new header-derived chain was replaced in `a81c1ca`; the one survivor is the booking placeholder chain (`process.env.NEXT_PUBLIC_APP_URL ?? process.env.VERCEL_URL ?? ''`, `src/app/portal/book/new/actions.ts:150`) — out of this section's scope, now given its named home as a go-live checklist §8 rider (added this pass) so it cannot evaporate.

### Second reviewer pass — 2026-07-22 (three refinements, all actioned)

The reviewer accepted the three-blocker disposition and returned three refinements. Each was verified against the code/config rather than conceded on authority, and each changed something:

1. **G-15 "one-line class of change" framing — corrected in place** (the G-15 entry above, being uncommitted work-in-progress, was edited rather than appended-to). The verified interaction analysis replaced the assertion: the reviewer's sanitizer concern was checked and **ruled out** (the proxy matcher already runs the sanitizer on every non-static route; `isProtected` gates only the redirect), the second-redirect-authority concern was checked and **confirmed as the real behaviour change** (seven prefixes, redirect authority moves from `requireRole` to middleware for the unauthenticated case only), and the residual maintenance coupling is named with its mitigation. The project's two prior understatement burns (G-13's grants restoration, `a81c1ca`'s call-site undercount) were the right precedent to invoke.
2. **G-4's evidence did not match G-4's named target — conceded and restated.** What was set is `sessions_timebox` (absolute session cap), not a refresh-token lifetime — a different Supabase setting, and the only one that exists; the run-log's "renamed control" phrasing was wrong and is corrected in the runbook in place (living doc). The ledger bullet above now argues explicitly that the time-box satisfies §12.3's intent *at least as strongly* rather than letting the reader infer the named target was hit. The Management API posture reversal (A.1(revised) Q4) is recorded: no new token was minted — the pre-existing Supabase CLI access token is the channel — and that credential, previously uncatalogued despite authorising production auth-config reads *and writes*, now has a `docs/secrets-inventory.md` entry (2026-07-22).
3. **The systemic finding, actioned same-day rather than deferred:** (a) the **single-ledger rule** is now written into `go-live-checklist.md`'s header — hardening/verification deferrals index on the checklist, feature deferrals in `deferred-prompts.md`, both updated in the same commit as any state change; polish docs become historical records at sign-off. (b) A **ten-doc drift audit** ran across every other closed section. Results: five clean (exercise-library, program-engine, program-calendar, ep-dashboard, client-portal-pwa), three lightly stale (scheduling — one closed-but-listed-open item; messaging — two unindexed feature deferrals; email-and-sms — its Sign-off list contradicted its own later Part B section, mirrored by an internal contradiction inside checklist §8 itself), and **two materially stale with this section's exact failure mode** (auth-onboarding-client — Pro-cutover closures and a decided open question never written back; client-profile-clinical-notes — CN-7 and the anon-EXECUTE sweep closed 2026-07-02 but still listed open, plus two deferrals indexed only in `rls-policies.md`, where their re-triggers could never fire). (c) **All findings reconciled same-day:** dated reconciliation notes appended to the four stale docs, the two rls-policies holes given a checklist §8 entry, the two messaging feature deferrals indexed in `deferred-prompts.md`, and the checklist §8 internal contradiction pruned. One **new actionable item** surfaced by the audit: the C-1/C-6 runtime verification in `auth-onboarding-client.md`, whose enabling trigger (a non-prod target) fired 2026-07-03 — unblocked, not yet run.

### Net position after this pass

G-6 stands as **partial build, staging-only** — prod apply rides the already-queued deploy sitting, and the G-6 record completes then. **G-15 is open** (deep-link reopened, awaiting approval of fix shape (a), correctly weighted). The ledger drift that produced this review cycle's false blockers is reconciled platform-wide and has a standing rule against recurrence. The reviewer's own standard applies to this pass too: G-15's closure gates on the browser test, and the C-1/C-6 verification gates on being run, not on this paragraph.

### Sign-off of the 2026-07-22 revisions — confirmed, with four residuals (all actioned same-day)

**Date:** 2026-07-22. **Reviewer:** claude.ai project chat (challenger role). **Decision:** the revision pass is confirmed — the three refinements landed properly. Four residuals were returned; disposition of each:

1. **The single-ledger rule conflicted with this pass's own behaviour** (this doc: signed off 2026-05-27, amended three times since — where would G-15's closure and G-6's prod apply be recorded?). **Fixed by naming the exception in the rule itself** (`go-live-checklist.md` header): *sign-off freezes the narrative, not reopened gaps* — a formally reopened gap or a deferred item advancing to a pending state records its closure as an appended, dated section in the owning polish doc (where the evidence chain lives), with the checklist entry updated in the same commit. Forbidden: re-editing the frozen narrative, or reading a sign-off-era deferred list as current state. Appending is the mechanism, not the violation.
2. **C-1/C-6 had no ledger entry** — the audit's one new actionable, recorded in prose in the very paragraph announcing the rule that requires indexing it. Conceded without qualification; it was the rule's named failure mode, committed by the rule's author in the rule's birth announcement. **Indexed:** `go-live-checklist.md` §8 now carries the entry (trigger fired 2026-07-03, verification unblocked and not yet run, owned by `auth-onboarding-client.md`), and that doc's reconciliation note cross-links it.
3. **"Five clean" had no stated method — and the method was the weaker one.** Stated honestly now: the audit's method was **ledger-consistency reconciliation** (each doc's deferred items checked against the checklist and runbooks), not per-item repo re-derivation. "Clean" meant *consistent with the authoritative ledger*, which catches a stale doc only where the ledger knows better — exactly the reviewer's concern. The concern was then tested, not disclaimed: the re-derivable open items across the five "clean" docs were checked against git, and **both flipped** — the program-engine doc's two spawned-task riders (SessionTypesEditor stale-add fix, test-05 fixture repair) shipped in `ac2f2d3` and `12cba90` on **2026-06-12, the same day that section signed off**, recorded nowhere. The audit verdict is downgraded accordingly: **four clean at repo level; program-engine ledger-stale on two same-day spawned-task closures** (reconciliation note appended to it). The residual un-re-derived items in the clean docs are bounded and named: signal-driven triggers with no state to re-derive (portal FM-4 offline; scheduling's off-tz drag), feature riders awaiting a build (portal video thumbnail; template management; builder decomposition), and ep-dashboard's doc-internal awaiting-sign-off states. Nothing else in those docs asserts a checkable past event.
...
**The record is partial, exactly as §"Reviewer revisions 2026-07-22" prescribed** — 8/10 events wired (`jwt.hook_failure` + `cross_tenant_access_attempt` have no emission point), 0/2 alert thresholds (no alerting pipeline). **The first-login behavioural check has since RUN and passed** (it could not run inside the sitting itself — no authed prod staff session existed at that hour, and the `.env.local` `PROD_SUPABASE_SERVICE_ROLE_KEY` used by the throwaway-staff probe pattern was found dead; the operator has since re-issued it): the operator's next prod login landed `auth.login.success` at **2026-07-22 22:45:01 UTC** (`detail.role=owner`), and the same session's invite send landed `auth.invite.sent` at **22:46:31 UTC** — both read back from `auth_events` via the prod workdir. Those two are the behaviourally-confirmed-on-prod pair the sign-off counts; the remaining six wired events are verified by byte-exact emitter-to-CHECK parity (the 2026-07-23 recon: every emitter literal present in the prod CHECK list, zero mismatches). **This record is complete; the sign-off is recorded below.**

---

## Sign-off — G-6 (2026-07-23)

**G-6 Structured auth-event audit log.**

- **Date signed off:** 2026-07-23
- **Reviewer:** Claude (Opus 4.8)
- **Decision:** Closed with deferred items (partial)

**Scope closed:** 8 of 10 events wired and verified (2 behaviourally confirmed on prod, 6 verified by byte-exact emitter-to-CHECK parity), 0 of 2 alert thresholds implemented. RLS enabled with zero policies on prod (relrowsecurity true, pg_policies count 0). Append-only enforced by BEFORE UPDATE OR DELETE row trigger keyed on session_user, refusing all application-reachable roles (anon, authenticated, service_role all authenticate as authenticator). Deferred items registered below.

### Deferred register (G-6)

- **D-1 Timezone:** prod apply and first-login checks are dated 2026-07-23 in this appendix; the evidencing auth_events rows carry occurred_at in UTC (e.g. 2026-07-22 22:45:01 UTC equals 2026-07-23 08:45:01 AEST, UTC+10). Rows do not predate table creation; the apparent gap is the UTC/AEST offset.
- **F-2 Alert thresholds, split by computability from currently collected data:** (a) greater-than-10 signup failures per hour is computable today (auth.signup.failure is logged with occurred_at); deferred item is the alerting pipeline only. (b) greater-than-50 login failures per hour per IP is NOT computable today; auth_events has no client-IP column and no emitter captures an IP; deferred item is a schema plus emitter change to capture client IP and THEN alerting; this is a migration, not a wiring job.
- **F-3 Append-only boundary:** the trigger keys on session_user and refuses UPDATE and DELETE for every application-reachable role; the postgres maintenance channel is exempt by design for retention. auth_events is append-only with respect to the application; it is NOT immutable against a database superuser or DBA. Append-only does not mean tamper-proof.
- **F-4 TRUNCATE blind spot:** a row-level trigger cannot fire on TRUNCATE; service_role retains TRUNCATE via Supabase default privileges; the sole control preventing audit-log erasure by service_role is that PostgREST exposes no TRUNCATE verb. Deferred hardening: REVOKE TRUNCATE ON public.auth_events FROM service_role. Re-trigger: any path that grants service_role direct SQL (edge function, RPC, SQL runner).
- **F-1 Attribution:** ON DELETE SET NULL on organization_id means audit rows survive org teardown but lose org attribution, and org teardown is itself the event most worth attributing. Deferred consideration: a denormalised organization_id_snapshot captured at insert. Recorded, not scheduled.
- **B-4 Audit-write failure routing:** logAuthEvent swallows INSERT failures by design so an audit miss can never block an auth flow (correct and to be retained). Today failures go to console.error only (Vercel function logs), not to the captureException observability seam that src/lib/comms/log.ts was moved onto at the section-12 gate. Consequence: when the real Sentry SDK lands, comms-log write failures will alert and auth-event write failures will not. Deferred fix: route the logAuthEvent failure path onto the same captureException seam. Re-trigger: when the real Sentry SDK is wired, or before identifiable client health data enters the project, whichever is first. Interim: audit-write failures are visible only in function logs, log completeness is best-effort, and no positive reconciliation or completeness proof exists at this scope.
- **Property note (not a defect):** auth.invite.sent writes the invited target address to the top-level email column on every send, so successive rows for the same detail.client_id in this append-only, teardown-surviving table distinguish a benign resend (same address twice) from an invite-address swap (two different addresses) from the log alone.
- **Q-1 Non-prod target:** verification used a genuine separate Supabase staging project (ref fbtfzlgvnivgwydlijka) distinct from prod (ref azjllcsffixswiigjqhj), the first G-section verified against a real non-prod environment. Resolved 2026-07-23: this staging project is the intended permanent non-prod solution, a persistent project and not a placeholder pending Supabase branching. Consequence: this partially addresses the standing no-non-prod-target liability. The target now exists; the residual risk is staging-to-prod schema parity over time, tracked at the standing-liability level rather than in this G-6 section. *(2026-07-23, same day, later sitting: that standing-liability tracking now actually exists — `go-live-checklist.md` §8 carries a named staging-to-prod schema-parity entry with its controls and re-trigger; until then this note pointed at a ledger entry that had never been written.)*

---

## G-6 deferred-register closures — 2026-07-23 parity pass (appended per the single-ledger exception)

The paying-client parity pass (operator goal: "complete parity for paying clients bar the external reviewing process") closed four of the register's items the same day the register was signed. Evidence chain here; `go-live-checklist.md` §8 G-6 entry updated in the same commit.

- **F-4 TRUNCATE blind spot — CLOSED.** Migration `20260723120000` (staging then prod): `REVOKE TRUNCATE ON public.auth_events FROM service_role`. postgres (owner) retains TRUNCATE for retention maintenance — the exemption the append-only trigger already models. The append-only property no longer depends on PostgREST's verb list. pgTAP `61` #9 asserts `has_table_privilege('service_role', …, 'TRUNCATE')` is false.
- **B-4 audit-write failure routing — CLOSED.** `src/lib/auth/events.ts` now routes both failure paths (DB error return AND thrown exception) through `captureException` with `{ where: 'auth-events:insert', event }` — the identical seam `src/lib/comms/log.ts` uses — so the Sentry SDK swap lights both up together. Best-effort posture retained: never rethrown, an audit miss can never block an auth flow.
- **F-2 alert thresholds — CLOSED (both halves).** (a) `client_ip inet` added to `auth_events` (`20260723120000`) and captured best-effort in `logAuthEvent` via request headers (x-forwarded-for first hop, x-real-ip fallback; NULL outside request scope; rows predating the column are honestly outside any per-IP window). (b) The alerting pipeline exists: `auth_events_threshold_scan(interval)` (`20260723150000` — SECURITY INVOKER on purpose: only service_role can both execute it and read auth_events; anon/authenticated EXECUTE stripped, pgTAP `61` #12–13) computes the two §11 thresholds over a trailing window; the `auth-events-alerts` Edge Function (hourly Vault-token pg_cron `auth-events-alerts-hourly`, migration `20260723170000`, verbatim mirror of the message-notifications cron pattern) emails `ALERT_EMAIL` on breach, fail-loud on missing config. A sustained attack re-alerts hourly by design — no dedupe state to go stale.
- **F-1 org-attribution snapshot — CLOSED.** `organization_id_snapshot` (bare uuid, no FK) stamped at insert by `logAuthEvent`; survives the org teardown that SET-NULLs the FK column. pgTAP `61` #11.

**Still honestly out, unchanged:** the two unwired events (`auth.jwt.hook_failure`, `auth.cross_tenant_access_attempt`) — no emission point exists without changing the G-1-verified hook's volatility or inventing a runtime RLS-denial detector; detection remains the G-1 probe + pgTAP 17/57; they re-enter scope with the external review (hard rule (a)). **Retention decision recorded:** no automated trim; append-only indefinitely at current volume; revisit near ~1M rows.

---

## G-15 closure — 2026-07-23 (fix shape (a), as cleared by the reviewer)

**Shipped exactly as approved:** the middleware's `isProtected` list (`src/lib/supabase/middleware.ts`) gained the seven staff route prefixes (`/analytics`, `/clients`, `/contacts`, `/library`, `/messages`, `/schedule`, `/settings`); the redirect carries `?next=<path>` (validated downstream by `safeNext`, unchanged); `requireRole` remains the sole authority for the claimless (`/onboarding/org`) and wrong-role (`/unauthorized`) branches. The named maintenance coupling ships with its mitigation: a G-15 REGISTRATION RULE comment sits beside `requireRole` in `src/app/(staff)/layout.tsx`, and the middleware list carries the mirror comment.

**The browser test G-14's commit claimed and never had — now exists and runs green** (`e2e/parity-pass.spec.ts`, clean-storage-state project against staging): (1) logged-out deep link to `/clients/<id>` → `/login?next=…` → real form login → lands on that exact client profile, not `/dashboard`; (2) the negative control — a claimless throwaway auth user driven through the same deep link. **The control's expectation was re-derived against the code on its first run:** the earlier "claimless still reaches /onboarding/org" phrasing conflated two branches. A claimless LOGIN lands `/portal` → `/unauthorized` by C-4's design (`postAuthLanding` null-role branch → the portal layout's onward route to the R-5 dead-end) — the deep link does not override it (asserted); `/onboarding/org` is `requireRole`'s branch for a claimless user NAVIGATING to a staff route, and is asserted at that real site in the same test. Torn down in `finally`. G-15 is **CLOSED**.

---

## Sign-off review 1 of 4 — reviewer verdict and revision (2026-07-23)

**Verdict (challenger reviewer, 2026-07-23): three of the five close; F-2 returned.** F-4/F-1 sound with notes; B-4 and G-15 closeable with conditions. No sign-off stanzas are recorded yet — the whole package goes back to the reviewer with this revision. Every blocking and conditional item was addressed the same sitting:

- **F-2(a) client-IP provenance — verified, then hardened anyway.** The reviewer's instruction was *verify, don't assume*. Verified against `vercel.com/docs/headers/request-headers` (last updated 2025-12-13): Vercel **overwrites** `x-forwarded-for` and "do[es] not forward external IPs … to prevent IP spoofing" — only the Enterprise *Trusted Proxy* add-on changes that — so the shipped first-hop read was **not** caller-controlled on this deployment today. The preferred fix ships regardless: `requestClientIp` (`src/lib/auth/events.ts`) now reads the platform-attested **`x-vercel-forwarded-for`** first ("identical to x-forwarded-for. However, x-forwarded-for could be overwritten if you're using a proxy on top of Vercel"), falling back to XFF first hop then `x-real-ip` only off-platform (local dev). A future proxy-in-front change can no longer silently poison the per-IP threshold. The preference order is pinned by vitest (`src/lib/auth/events.test.ts`, F-2a describe block).
- **F-2(b) sampling seam — conceded, fixed, and LIVE IN PROD.** Migration `20260723180000` moves the cron to `7,22,37,52 * * * *` — 4× sampling of the unchanged 60-minute window, exactly as prescribed. Any burst of ≤45 minutes' duration now lands whole in at least one scan; the residual is an attacker deliberately spreading across the full hour, which shaves the effective threshold to ~1.33× at worst — accepted and recorded here. **Re-alert cadence re-reasoned (stays dedupe-free):** a sustained breach now emails up to 4×/hour instead of 1×/hour — accepted, because during an active incident that is pager signal, not noise, and dedupe state that can go stale (silently suppressing a real alert) remains the worse trade. The jobname keeps its `-hourly` suffix for jobid continuity; the cadence lives in the schedule, not the name. **Prod-applied 2026-07-23** (via the prod workdir, pushing only `20260723180000` — the sibling's unrelated `20260723190000` was removed from the throwaway workdir copy so it did not ride along); prod `cron.job` read-back is the reviewer's required evidence: `jobname=auth-events-alerts-hourly, schedule='7,22,37,52 * * * *', active=true`. The property the doc asserts is now the property production has.
- **F-2 negative — asserted live on prod (2026-07-23).** POST with no bearer → **401**; POST with a wrong bearer → **401**, body the bare string `unauthorized` (no scan telemetry leaks to an unauthenticated caller — the counts/breaching-IPs body is reachable only behind the Vault bearer); GET → **405**. `verify_jwt = false` confirmed in `config.toml`, making the in-function bearer check the sole barrier — which is why it fails **closed** (500) on an unset secret.
- **B-4 — executed verification now exists; residual honestly widened.** `src/lib/auth/events.test.ts`: the DB-error return routes through `captureException` with `{where, event}` context; a thrown exception routes with the original error; the success path never touches the seam; `logAuthEvent` resolves (never rethrows) in all cases. **Disclosed residual (was omitted):** B-4 closed *routing*, not *observability* — `captureException` is still the `console.error` stub (`src/lib/observability/sentry.ts`), so a prod audit-write failure is log-visible, not alerting, until the Sentry SDK lands. This joins the "still honestly out" list beside the two unwired events.
- **G-15 — comment downgraded to a note; the coupling is now machine-checked.** The prefix list moved to the pure module `src/lib/supabase/protected-routes.ts` (middleware consumes `isProtectedPath`; behaviour unchanged, e2e tests 1–2 still cover it). `protected-routes.test.ts` asserts `STAFF_ROUTE_PREFIXES` **equals** the `src/app/(staff)/` route-directory set in both directions on every `npm test`: a new staff route missing from the list fails the suite (the G-15 hole), and a stale prefix with no directory fails it too. The REGISTRATION RULE comments remain as signposts only.
- **F-4 notes — verified on staging.** `service_role` is a member of **no** roles (`pg_auth_members` empty for it), and `has_table_privilege` includes membership chains by default — so pgTAP 61 #9's FALSE already covers membership-derived grants; the only TRUNCATE holder on `auth_events` is `postgres`. The named silent-revert risk (`GRANT ALL ON ALL TABLES … TO service_role` would restore TRUNCATE with no error) is recorded: pgTAP 61 #9 is the standing tripwire, run per-migration with the suite.
- **F-1 prod live-capture — CLOSED on prod (both halves captured; the reviewer's suggested probe corrected first).** A real prod login (2026-07-23 21:49 UTC) captured `client_ip = 101.191.183.19` on the `auth.login.success` row — **F-2b prod-proven**, backed by staging 18/18 IP-carrying over the trailing 12 h. The reviewer's "confirm the snapshot on a login row" was mis-specified: `logAuthEvent('auth.login.success')` (`src/app/login/actions.ts`) passes no `organizationId` (at login the role/org is not yet resolved into the call), so login rows carry `organization_id` **and** `organization_id_snapshot` null by construction. The **only** emitter that passes an org id is `auth.invite.sent` (`src/lib/clients/invite.ts`), where the snapshot and `client_ip` are the same INSERT (`organization_id_snapshot: fields.organizationId ?? null` beside `client_ip`). **The snapshot was proven on prod with one throwaway invite (2026-07-23 21:55:42 UTC, `delivered@resend.dev`):** `client_ip = 101.191.183.19` non-null **and** `organization_id_snapshot = 33d23c20-4c41-42c9-8918-ec663895ea56` = `organization_id` (`snapshot_matches = true`). **Teardown ran zero-residue** (leaf→root inside a transaction, comms → invite_tokens → clients; a `client_id`-column scan across all public tables found exactly those three child/parent rows, no auth.users/roles/threads since the invite was never accepted): post-commit census `delivered@resend.dev` clients = 0, communications = 0, invite_tokens = 0; the append-only `auth_events` invite.sent row is retained as the evidence. Staging + pgTAP `61` #11 already proved the co-write; this closes the staging→prod inference gap the reviewer named. *(Leg-2 reconciliation: the two pre-existing prod invite rows are `2026-07-22 22:46 / 23:45 UTC` = `2026-07-23 08:46 / 09:45 AEST` — the §12 "Leg 2 ran 2026-07-23" record is the 22:46 UTC `delivered@resend.dev` row, present as claimed. Its snapshot is null only because it predates the snapshot-writing deploy — a UTC-vs-AEST date-label artifact, not a missing row; nothing to fix.)*

**Two register notes from the reviewer (non-blocking), actioned:**

- **EF bearer compare is now constant-time.** `authorizeCronRequest` (`supabase/functions/auth-events-alerts/index.ts`) compared the bearer with plain `!==`, which can leak via early-exit timing how many leading bytes matched (not remotely exploitable, but not the correct form for a secret compare). Replaced with a length-guarded XOR-accumulating `timingSafeEqual` over the full span. The `send-appointment-reminders` / `send-message-notifications` mirrors still use plain `!==` and carry the same one-liner for whenever those files are next opened (deliberately not swept here — they are other sections and separate EF deploys). **Deployed to prod 2026-07-23** (`supabase functions deploy auth-events-alerts`); the negative probes re-ran green against the redeployed function (no bearer → 401, wrong bearer → 401 `unauthorized`, GET → 405), confirming the constant-time path rejects correctly.
- **Dedupe-free 4×/hour alerting can meet Resend rate limiting or a spam filter — silently.** Added to the residual list (and the EF header comment): a very long sustained breach re-alerting 4×/hour will eventually hit provider throttling/filtering, and that suppression is silent (a Resend non-2xx is `console.error`'d, but a filter-drop is not observable here). Accepted at f&f scale; revisit with a delivery-health check if alert volume ever justifies it.

**Revision verification run (STAGING unless stated):** vitest **9/9** (3 files — B-4 ×3, F-2a provenance ×2, G-15 coupling ×1, plus the pre-existing RO-6 ×3); `tsc --noEmit` clean; `next build` exit 0; migration `20260723180000` applied to **staging** (linked ref `fbtfzlgvnivgwydlijka`), then **prod** (ref `azjllcsffixswiigjqhj`), prod `cron.job` read-back `7,22,37,52 * * * *` active; pgTAP `61` re-ran **14/14** on staging; the 401/405 probes + the F-1 login/invite capture + zero-residue teardown ran against prod (read-only reads + one fully-torn-down throwaway invite).

---

## Sign-off — Sign-off review 1 of 4 (G-6 register closures F-4/F-2/F-1/B-4 + G-15)

- **Date signed off:** 2026-07-23
- **Reviewer:** claude.ai challenger chat ("Sign-off review 1 of 4")
- **Decision:** Closed with deferred items. Signed off with one condition — that the F-2 stanza not read CLOSED while prod still scanned hourly; the condition was met the same sitting by prod-applying `20260723180000` and capturing the prod `cron.job` read-back (`7,22,37,52 * * * *`, active). F-2(a)/(b) design, the 401/405 negative, B-4, G-15, F-4, and F-1 all cleared; the reviewer recorded the correction that their F-1 login-row probe was wrong (login rows carry null org by construction) and that the invite-path proof was the right call.

**Deferred items (re-triggers):**
- The two unwired G-6 events (`auth.jwt.hook_failure`, `auth.cross_tenant_access_attempt`) — no emission point without changing the G-1-verified hook / inventing a runtime RLS-denial detector; re-enter with the external review (hard rule (a)).
- **B-4 observability** — routing is closed but `captureException` is the `console.error` stub; an audit-write miss is log-visible, not alerting, until the Sentry SDK lands.
- **Constant-time EF bearer compare — DONE** (deployed to prod 2026-07-23, negatives re-probed green); listed here because it was pending at the moment of sign-off, retained for the record.
- **Silent alert-suppression under sustained-breach throttling** (Resend rate-limit / spam filter) — accepted at f&f scale; revisit with a delivery-health check if volume justifies it.
- **`auth_events` retention** — no automated trim; append-only indefinitely at current volume; revisit near ~1M rows.

**Remaining operator step — DONE 2026-07-23:** the `auth-events-alerts` EF was redeployed to prod (`supabase functions deploy auth-events-alerts --project-ref azjllcsffixswiigjqhj`), carrying the constant-time bearer compare (and the comment-only cadence-doc changes); negatives re-probed green. No open items remain for this review.
