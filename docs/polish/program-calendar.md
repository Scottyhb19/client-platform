...
> **Status update (2026-06-14):** the table below is the **2026-06-12 audit snapshot**, retained verbatim as the pre-pass record — do not read its statuses as the current state. Five rows have since been resolved by the pass and are no longer open: *Calendar fills full screen width* → **P2-4**; *Program label with an Active tag* → **P2-5**; *Copy Week / Repeat Specific buttons* → **Q1 / P1-1** (week ops on the collapsed week row); *"Today" snap-back FM-1 caveat* → **P0-2**; *chevron-only collapse affordance* → **P2-3**. See §7 (progress log) and §8 (closing commit) for the closure detail.
...
- **Round-two walkthrough (operator, 2026-06-12): all five issues confirmed fixed** ("Everything works"). One layout request remained → P2-6 below.

### P2-6 — Header rearrangement: block actions behind one menu (closed; type-check/lint/build clean 2026-06-12)

Operator at the round-two re-test: the four block-action buttons (Copy / Repeat / Archive / Save as template) plus New training block + panel toggle squashed the "Program Calendar" title into a three-line wrap. Requested a single **"Training block actions"** dropdown holding the four block-scoped actions.

- **Fix:** [`ProgramToolbar.tsx`](../../src/app/(staff)/clients/[id]/program/_components/ProgramToolbar.tsx) replaces the four inline buttons with one `btn outline` trigger (`Layers` icon + label + `ChevronDown`) opening a `role="menu"` of four `ActionMenuItem` rows that fire the existing `mode` transitions. Esc + outside-click close it. **New training block** stays a separate primary button — it creates a block rather than acting on one, so it doesn't belong in a "current block" menu. The header now reads: title · [Training block actions ▾] · [New training block] · [panel toggle].
- **Design-system conformance:** the menu carries **no shadow** (the system reserves the one subtle shadow for cards — "no shadows on menus"); separation is a 1px `--color-border-subtle` + `--color-card` background, 10px panel radius, 7px item radius, subtle `--color-surface` hover at 150ms. **Casing:** sentence case ("Training block actions", "Copy block", …) per the CLAUDE.md voice rule, in deliberate deviation from the operator's title-case phrasing; flagged to the operator. "Save as template" kept over "Save block template" (the established, clearer label).
- **Verification:** type-check, lint (no new findings), `next build` clean; route serves; live on the dev server via HMR for the operator. Visual sign-off pending operator.
...
## 8. Closing commit — Program calendar (section 6)

Section-level closing commit for the documentary review, covering all twelve gaps (P0-1, P0-2, P1-1…P1-4, P2-1…P2-6) plus the `_test_*` fixture rider closed en route. Per-gap detail and provenance are in §7; this section is the consolidated record. Build order followed the protocol (architecture → features → polish; exact order in §6). Final gate state: pgTAP green on live (full suite 176/176; the three calendar suites re-confirmed 2026-06-14), `type-check` / `eslint` / `next build` clean, and operator visual confirmation complete (P1-3 across two rounds; P2 on 2026-06-14).

### What changed, by gap number

- **P0-1 — Anon-EXECUTE sweep of the calendar RPC family (FM-2).** Migration `20260612150000` revokes `anon` EXECUTE on the ten guarded caller-facing RPCs and makes the guardless internal `_program_for_date` definer-only (mirroring `_clone_program`). A live grant-probe (not migration-file enumeration) caught one function the audit had missed (`duplicate_program_day`). pgTAP `23_program_rpc_grants.sql` is the regression tripwire for the Supabase auto-grant trap. The calendar-family portion of the go-live rider is discharged; the full-platform sweep stays indexed in `docs/go-live-checklist.md`.
- **P0-2 — Practice-timezone "today" foundation (FM-1).** `todayIsoInPracticeTz()` ([`src/lib/dates.ts`](../../src/lib/dates.ts)) derives the ISO date in `PRACTICE_TIMEZONE = 'Australia/Sydney'` ([`src/lib/constants.ts`](../../src/lib/constants.ts)), replacing UTC `new Date().toISOString()` at three call sites (calendar loader, new-block default start, `SingleDatePicker`). Fixes the "today is yesterday until ~10–11am AEST" class across the today ring, Today snap-back, copy-pick past-dimming, and `resolveCurrentBlock`. Rider out: the client-portal week-strip shares the pattern → recorded to section 7, not fixed here.
- **P1-1 — Copy week + Repeat week (FM-5; Q1 = a, amended).** Migration `20260612160000`: `copy_program_week` / `repeat_program_week`, orchestrators over the G-1-verified `copy_program_day` (per-set fan-out, superset remap, week-wide conflict accumulation surfaced in one dialog). Affordances on the **collapsed week summary row** (Q1 amendment — not new toolbar buttons); week-copy reuses the day-copy state machine at row granularity, week-repeat reuses `RepeatEndDatePicker` in `weekMode`. pgTAP `24` (20/20). "Repeat Specific" is satisfied by the existing day-level repeat-weekly flow.
- **P1-2 — Honest error surfacing on all calendar mutations (FM-3).** New `error-toast` mode; the four day-level runners migrated off their silent-`console.error` / mislabeled-no-program-toast paths to a factual one-button dialog. `ProgramToolbar`'s block-level ops were already compliant via `ErrorDialog`.
- **P1-3 — Browser pass of all clone paths (FM-4; discharges section-5 carried items a + b).** Operator-run authenticated walkthrough, two rounds. Round one surfaced three real defects (→ P1-4 issues 1–3); round two after the fixes returned **"Everything works"**, plus two further defects caught and fixed (→ P1-4 issues 4–5). All six clone paths — copy day, repeat day weekly, week copy, week repeat, block copy, block repeat — have their end-to-end browser pass.
- **P1-4 — Block copy/repeat defects from the walkthrough (five issues, each root-caused from the live DB before any fix).** (1) Overlap-detail payload naming the colliding block(s) by date — migration `20260612170000`; (2) archived block lingering on the client profile — shared `resolveCurrentBlock` extracted to [`src/lib/programs/current-block.ts`](../../src/lib/programs/current-block.ts), the multi-active `.maybeSingle()` error fixed; (3) copy source-block selector (`BlockSourceField`); (4) future-only block now resolves (step-3 earliest-upcoming fallback) and archive gains a which-block selector; (5) copy weekday misalignment — migration `20260612180000` aligns the picked start to the source-start weekday so every session keeps its weekday. pgTAP `11` (16/16) + `23` (32/32).

- **P2-4 — Full-width calendar by default (FM-9, Q3 = a).** [`page.tsx`](../../src/app/(staff)/clients/[id]/program/page.tsx): the wide container (`maxWidth: min(2000px, 98vw)`, 8px horizontal padding, top/bottom 32px inherited from `.page`) now applies in **both** panel states, not just panel-open. The closed-state 1200px `.page` cap was inherited from the standard staff container — Phase E.0a only ever decided the panel-open width. §6.2 ("fills full screen width by default") now met. The day popover sizes to cell width, so wider cells only give it more room; the week-row buttons are unaffected.
- **P2-1 — Block-boundary cue (FM-6).** [`MonthCalendar.tsx`](../../src/app/(staff)/clients/[id]/program/_components/MonthCalendar.tsx) `DaySummaryPopover`: a quiet block-name eyebrow (font-display, weight 600, .62rem, tracked, uppercase, muted; truncates with a `title` fallback) now renders above the day-summary header, so any opened day self-identifies its training block. The `program` prop was already threaded in. The optional 1px grid boundary hairline was **declined** on the restraint bar — the design system's anti-decoration posture and the restricted left-border-accent rule argue against a passive grid divider; the popover eyebrow plus the existing "N blocks" month eyebrow carry the cue. Re-trigger: operator finds the at-a-glance boundary genuinely ambiguous in real use.
- **P2-3 — Week-collapse affordance widening (FM-7).** `MonthCalendar.tsx` `MonthGrid`: extracted a shared `toggleWeek(ord)` helper; the chevron gutter now stretches to a full-height click target in the expanded state, and the **entire collapsed summary strip** toggles the week. The Copy/Repeat week buttons `stopPropagation`, so they still act independently; both new targets are suppressed during pick modes so they can't hijack an in-progress copy. `title` tooltips added for discoverability. **Default stays expanded** — recorded accepted deviation from §2.1 ("weeks minimised by default"): collapsed-by-default would hide the month at a glance in the operator-validated single-month grid.
- **P2-5 — Active tag on the current-block label (§6.2).** `page.tsx`: a quiet `.tag.active` chip (accent-green — a sanctioned success-state use of the green) now leads the current-block meta line under the title; the block name is in the eyebrow directly above. **Nuance surfaced:** archived blocks are already excluded from this surface, so every block shown here is active — the tag is confirmation, not disambiguation, but the brief explicitly calls for "an Active tag," so it is added.
- **P2-2 — Design-token sweep (FM-10).** Replaced the duplicated inline colour constants and the cleanly-mappable radii with token references:
  - [`CalendarSidePanel.tsx`](../../src/app/(staff)/clients/[id]/program/_components/CalendarSidePanel.tsx): removed `INK`/`MUTED`/`CREAM_DEEP` → `--color-primary` / `--color-muted` / `--color-surface-2`; `#fff` → `--color-card`; tab-strip `7` → `--radius-button`. (Micro-shift: `MUTED #78746F` → `--color-muted #7a7166`, which the design system documents as the darkened replacement for the original `#78746f`.)
  - [`CalendarPanelToggle.tsx`](../../src/app/(staff)/clients/[id]/program/_components/CalendarPanelToggle.tsx): removed `BORDER`/`INK`/`MUTED`/`ACCENT` → `--color-border-hairline` / `--color-primary` / `--color-muted` / `--color-accent`; `#fff` → `--color-card`; `7` → `--radius-button`.
  - `MonthCalendar.tsx` & [`ProgramToolbar.tsx`](../../src/app/(staff)/clients/[id]/program/_components/ProgramToolbar.tsx): popover/dialog radii `10 → --radius-card-dense`, `14 → --radius-card`, clean `7 → --radius-button` / `--radius-input`. The accepted floating-overlay shadows (`0 12px 28px` popover, `0 24px 60px` dialog — Phase F P2-4) were **kept and named** in comments, not deleted (they are legitimate overlay elevation, not the banned button/menu shadow; no shadow token exists).
  - **Amber normalization (deliberate, surfaced):** the warning icon in `ErrorDialog` (was a bare `#d97706`) and in `ConflictDialog` (was `var(--color-warning, #d97706)`) both now reference `var(--color-warning)` (#e8a317). `--color-warning` already exists, so this is using an existing token, not inventing one. ConflictDialog already resolved to the token via its fallback chain (its icon was already #e8a317); ErrorDialog rendered the off-token #d97706 — the two dialogs were inconsistent on the live site. Now aligned; **ErrorDialog's warning icon shifts slightly toward the system amber — flagged for operator visual confirmation.**
  - **Surfaced, NOT changed** (no token exists; adding one edits the design-system layer — an operator/design decision per the handover, recorded with a `// P2-2 TODO` at the cream site): the calendar cream surface `#f5f0ea`; the accent tints at 4/6/8% alpha (only 10% is tokenised as `--color-accent-soft`); the modal scrim `rgba(28,25,23,0.5)` (×3); the source-block dim `rgba(28,25,23,0.04)` and the alert-alpha icon-delete tints; `#fff`-on-accent contrast text on picked dates; off-system 5/6/8px radii. Broad token consolidation remains the future design-system-layer pass already recorded in this doc.
- **P2-6 — Header rearrangement (operator request, sign-off log side note).** The four block actions (Copy / Repeat / Archive / Save as template) collapsed into one **"Training block actions"** dropdown (`ActionMenuItem`), un-squashing the "Program Calendar" title; New training block stays a separate primary button (it creates a block rather than acting on one). The menu carries **no shadow** (design system: menus carry no shadow), sentence-case labels.
- **Rider closed en route — `_test_*` pgTAP fixture helpers locked down.** [`00_test_helpers.sql`](../../supabase/tests/database/00_test_helpers.sql) is self-securing (an in-body `session_user = 'postgres'` guard that survives the auto-grant trap); migration `20260612160100` revoked the residual PUBLIC grant. Full scriptable suite re-run after the lockdown: **176/176 across 15 files**. `docs/go-live-checklist.md` §4 finding resolved.

**Opportunistic pre-existing lint fixes (NOT P2 — surfaced).** eslint on the touched files surfaced two pre-existing findings in code the P2 edits did not touch (confirmed absent from the diff): `let exercisesByDayId` (prefer-const, error) in `page.tsx`, and a dead `iconBtnStyle` const in `MonthCalendar.tsx` (its only real consumers live in `TestCaptureModal.tsx`, which has its own local copy). Both fixed — zero behaviour change. This makes `npx eslint <touched files>` report **only** the one sanctioned pre-existing react-hooks `set-state-in-effect` error (the month-reset effect), exactly as the handover anticipated.

### Acceptance tests run + results

- **pgTAP (live):** full scriptable suite **176/176 across 15 files** (09–13, 15–24) at the rider closure (2026-06-12); the three calendar-owned suites re-confirmed green on **2026-06-14** — `11_program_copy_repeat` 16/16, `23_program_rpc_grants` 32/32, `24_program_week_copy_repeat` 20/20, zero failures. Tests 01–08/14 predate the `_tap` scriptable pattern and are unchanged by this section.
- **Build gates (P2 commit `09915ed`):** `npm run type-check` clean; `npx eslint` on the touched files → only the one sanctioned pre-existing react-hooks `set-state-in-effect` error (the MonthCalendar month-reset effect); `npm run build` exit 0. (Two pre-existing lint findings in untouched code — a `prefer-const` and a dead style const — were fixed opportunistically; see the note above.)
- **Browser (operator, authenticated):** P1-3 two-round walkthrough — all six clone paths confirmed (**"Everything works"**). P2 polish — visual confirmation on **2026-06-14** of the full-width layout, the Active chip, the day-popover block eyebrow, the widened collapse targets, and the ErrorDialog warning-icon colour.
- **Runtime smoke (P2):** the authenticated program route returns 200 after every HMR recompile (operator's live session, per dev-server logs); the unauthenticated preview browser redirects to `/login` (not a 500).

### Premortem failure modes — FM-1…FM-10 disposition

Every premortem failure mode from §2 is dispositioned:

- **FM-1** (UTC "today") → **mitigated, P0-2**. **FM-2** (guardless anon-reachable definer) → **mitigated, P0-1**. **FM-3** (silent / mislabeled errors) → **mitigated, P1-2**. **FM-4** (clone fan-out unverified in the UI) → **mitigated, P1-3**. **FM-5** (missing week-level batch op) → **mitigated, P1-1**. **FM-6** (block boundaries invisible) → **mitigated, P2-1** for the open-day case via the popover eyebrow; the passive grid hairline **deliberately declined** on the restraint bar (re-trigger: at-a-glance boundary proves ambiguous in real use). **FM-7** (collapse affordance undiscoverable) → **mitigated, P2-3** via widened targets + tooltips; default-expanded **deviation accepted** (rationale recorded). **FM-8** (pin/view-mode localStorage-only) → **accepted, Q2 = a** (f&f-beta scope; re-trigger: a second device/practitioner in real use, or a Settings preferences surface). **FM-9** (full-width unfulfilled) → **mitigated, P2-4**. **FM-10** (design-token drift) → **mitigated, P2-2** for the cleanly-tokenisable subset; design-layer-decision values surfaced.

### Deferred / surfaced — with re-triggers

- **P2-2 design-layer values** — the calendar cream `#f5f0ea`, accent tints at 4/6/8% alpha, the modal scrim, ink-dim tints, `#fff`-on-accent contrast text, and off-system 5/6/8px radii were **surfaced, not invented** (a new token edits the design-system layer, an operator/design decision). Re-trigger: the broad design-system-layer token pass already recorded in this doc, or an operator token decision.
- **Accepted, carried forward unchanged (re-triggers in §2 "Accepted"):** the Q-D concurrency acceptances; the `MonthCalendar.tsx` ~2,000-line monolith; mouse-only copy/week-pick (no keyboard path beyond Esc); the loader fetching the client's full active history.
- **Open, but outside this section (indexed in `docs/go-live-checklist.md`):** the platform-wide SECURITY DEFINER anon-EXECUTE sweep; the non-production test target.

### Status

All twelve gaps — P0-1, P0-2, P1-1, P1-2, P1-3, P1-4, P2-1, P2-2, P2-3, P2-4, P2-5, P2-6 — are implemented, tested, and operator-visually-confirmed (P1-3 across two rounds; P2 on 2026-06-14). No gap is deferred; the deferred/surfaced items above are design-layer or out-of-section, each with a re-trigger. **Not self-certified closed** — per the section sign-off ritual, this closing commit goes to the claude.ai project chat, and the decision (Closed / Closed with deferred items / Returned for revision) returns under a "Sign-off" heading below.
