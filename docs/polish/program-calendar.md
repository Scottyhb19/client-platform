...
### P1-4 — Block copy/repeat defects from the P1-3 walkthrough (closed; pgTAP 11 14/14 + 23 32/32 green on live, type-check/lint/build clean 2026-06-12; re-walkthrough pending operator)

Three operator-reported issues, each root-caused before any fix (no symptom-patching):
...
- **Verification:** pgTAP 11 14/14 and 23 32/32 green on live; type-check, lint (no new findings), and `next build` clean; the calendar route serves (redirects unauth, no 500). The visual confirmation of the dropdown + overlap-detail dialog needs the operator's authenticated session — folded into the P1-3 re-walkthrough.
