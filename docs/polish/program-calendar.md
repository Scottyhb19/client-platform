...
## 8. Closing commit — P2 polish (full-width, boundary cue, collapse affordance, Active tag, token sweep)

Type-check + lint + `next build` clean 2026-06-14. **Operator visual sign-off pending** (the authenticated walkthrough is the operator's step). UI-only — no migrations, no SQL, pgTAP unchanged. Build order followed the handover (P2-4 first as the width baseline, P2-2 last as it touches everything the others changed).
...
- `npx eslint` on the five touched files — only the sanctioned pre-existing react-hooks `set-state-in-effect` error (MonthCalendar month-reset effect) remains; no new findings.
- `npm run build` — exit 0 (full route manifest emitted).
- **No pgTAP** — P2 is UI-only; no migration, no RPC, no grant change.
- **Runtime smoke:** the authenticated program route returns **200 after every HMR recompile** of these edits (operator's live session, per dev-server logs — `✓ Compiled` then `GET …/program 200`); the unauthenticated preview browser correctly redirects to `/login` (not a 500). Visual aesthetic sign-off is the operator's step.

### Premortem failure modes — mitigated vs accepted

- **Mitigated:** FM-9 (full-width unfulfilled) → closed. FM-6 (block boundaries invisible) → mitigated for the open-day case via the popover block eyebrow; the passive grid-line mitigation deliberately **accepted-as-not-done** on the restraint bar (re-trigger above). FM-7 (collapse affordance undiscoverable) → mitigated via widened click targets + tooltips, with the default-expanded **deviation accepted** (rationale recorded). FM-10 (token drift) → mitigated for the cleanly-tokenisable subset; the design-layer-decision values surfaced for a future pass. The §6.2 "Active tag" conformance gap closed.
- **Accepted (carried forward unchanged, with re-triggers in §2 "Accepted"):** the Q-D concurrency acceptances; the `MonthCalendar.tsx` ~2,000-line monolith (marginally larger now); mouse-only copy/week-pick (no keyboard path beyond Esc); loader fetches the client's full active history. Still open and indexed elsewhere (NOT section-6 gaps): the platform-wide SECURITY DEFINER anon sweep and the non-prod test target (`docs/go-live-checklist.md`).

### Status

P2-1 / P2-2 / P2-3 / P2-4 / P2-5 implemented and gated; P2-6 closed earlier in this pass. **Not self-certified closed.** Per the section sign-off ritual: the operator runs the authenticated visual walkthrough on the dev server, then pastes this closing commit into the claude.ai project chat; the sign-off decision returns under a "Sign-off" heading below.
