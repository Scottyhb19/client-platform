...
| Date | Run by | Result |
|---|---|---|
| 2026-07-02 | Claude Code (automated re-run) | **All checks pass (8/8).** Re-ran the automated `supabase/tests/database/17_cross_tenant_isolation.sql` via `supabase db query --linked` against the live project, immediately after the legacy→new Supabase API-key migration (a value swap that touched no RLS policy, JWT hook, or tenant-table shape). `ok 1-8`: read isolation (org_b sees zero of org_a's client / clinical_note / program), write isolation (`UPDATE` of an org_a client affects 0 rows; `INSERT` carrying org_a's `organization_id` raises `42501`), and controls proving real isolation rather than a blind session. Self-fixturing (creates and rolls back its own two orgs + two staff); no manual signup or teardown. |
