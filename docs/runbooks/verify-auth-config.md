...
- **G-4:** Authentication → Sessions → **"Time-box user sessions"** (Management API field `sessions_timebox`) → 30 days / 720 h. **Correction 2026-07-22 (staff-auth reviewer pass):** the earlier parenthetical here called this "the dashboard renamed the old 'Refresh token max lifetime' control" — wrong. The time-box is a **different setting** (absolute session cap regardless of activity), not a rename; no configurable absolute refresh-token-lifetime control exists on the current dashboard. The time-box is what was set, and it satisfies §12.3's "uniform 30-day session" intent at least as strongly as the originally-named target: rotation kills stolen-token replay (behaviourally verified 2026-07-02), and the time-box caps even an actively-refreshed session at 30 days — which a refresh-token idle-expiry alone would not. G-4's recorded target is therefore restated as `sessions_timebox = 720 h` + rotation on + reuse interval 10 s. Confirm rotation on, reuse interval 10 s. **This is the documentation-only value — record it here when set:** **SET 2026-07-21** — operator set 30 days at the Pro cutover; verified same day by Management API re-read (`sessions_timebox: 720` hours, rotation `true`, reuse interval 10 s). Sessions are 30-day uniform across roles (recorded decision, revisit at the paying-client gate).
...
| 2026-07-21 | GREEN (script) | **GREEN (script — first behavioural pass ever).** Front-door signUp rejected the corpus-confirmed breached password with `pwned` | **GREEN (script — the definitive C-7 answer).** `updateUser` REJECTED the breached password with `pwned` — HIBP covers the path every client welcome/reset uses. **C-7 closes as verified, no code change** | CND (script — `email_address_invalid` on the probe address, the ⚠️ predicted case). Setting confirmed ON same day via Management API: `mailer_autoconfirm=false` | DOC — Management API read same day: rotation `true`, reuse interval `10s`; `sessions_timebox` was `0` at first read, operator set 30 days in-sitting, **verified by re-read: `720` hours** | **Pro-cutover run.** HIBP enabled by the operator on the newly-upgraded Pro plan (Management API read confirmed `password_hibp_enabled=true`, `password_min_length=12`). G-3's plan-gate is lifted; the "expected RED on free tier" reading of G-3u is obsolete — from this row onward G-3u RED means the real C-7 platform hole. Healthy Pro steady state achieved: G-1/G-3/G-3u GREEN. G-7's probe-domain blocker persists on Pro (email validation, not the free-tier mailer rate limit) — setting-level verification via Management API remains the G-7 method. **G-4 note:** the dashboard control formerly sought as "Refresh token max lifetime" is now **Authentication → Sessions → "Time-box user sessions"** (Management API field `sessions_timebox`); target 30 days (720 h), per the recorded session-duration decision (30-day uniform across roles at f&f scope; revisit at the paying-client gate). |
