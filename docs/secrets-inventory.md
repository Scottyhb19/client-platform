...
| `NEXT_PUBLIC_APP_URL` | Reminder-email link base (Edge Function ONLY since 2026-07-23 — app code no longer reads it; `NEXT_PUBLIC_SITE_URL` is canonical via `getPublicOrigin()`) | `supabase/functions/send-appointment-reminders/index.ts` | Edge Function secret (Vercel copy retained + sync-checked until an operator env sitting removes it) | Public app address |
| `ALERT_EMAIL` | Destination inbox for the hourly auth.md §11 threshold alerts (G-6 F-2, 2026-07-23) | `supabase/functions/auth-events-alerts/index.ts` | Edge Function secret | A destination address, not a credential — but a wrong value silently redirects security alerts, so verify on change |
...
**`NEXT_PUBLIC_SITE_URL` vs `NEXT_PUBLIC_APP_URL` — failure mode (Flag E), LARGELY RESOLVED 2026-07-23.** Same logical value, two different keys. ~~Signup uses SITE_URL; booking and reminder emails use APP_URL.~~ The 2026-07-23 origin-idiom consolidation made `NEXT_PUBLIC_SITE_URL` canonical for ALL app code (every email-link origin now goes through `getPublicOrigin()`, fail-loud); the ONLY remaining `NEXT_PUBLIC_APP_URL` reader is the reminder Edge Function's own secret store. Residual invariant: the EF's copy must match the canonical origin (a mismatch breaks only the reminder email's button link). **RESOLVED in full 2026-07-23 (later sitting):** the operator removed the Vercel `NEXT_PUBLIC_APP_URL` copy and `required-env.ts` dropped it from the required list in the same change-pair (the interim state — var removed while still required — briefly showed as `/api/health` 503 `missing_env`, exactly the designed detection, cleared by the paired deploy). Residual: the reminder Edge Function's own secret-store copy, asserted by the EF itself at send time. **Verified 2026-07-03 (historical):** Vercel had both keys set + in sync; the `.env.local` shadowing-duplicate pair was removed the same day.
