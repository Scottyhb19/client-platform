...
- **A backup restore actually exercised once** — not "PITR is available on the plan," but a restore proven on a scratch project, documented in `docs/disaster-recovery.md`, even at f&f scale. **STILL OPEN — blocked on plan (confirmed 2026-07-02: project is on Free; "Restore to a New Project" is Pro-only).** The drill recipe is now written (`docs/disaster-recovery.md`); run it as a ~30-minute follow-the-steps task right after the Pro upgrade (itself a go-live step) and log the result there. Documented dependency, not a skip — the only remaining Beta-entry gate item.
