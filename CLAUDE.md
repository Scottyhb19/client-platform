...
## Project state
**Phase 1 is functionally complete and the polish pass is complete.** All 14 steps from the original build order have working code in the repo, and all 12 locked polish sections are closed. The platform is operational. It is now **entering the friends-and-family beta** — the transition from "only fake/seed data, no real client has logged in" to real friends-and-family users with real health data in the system. That transition is gated by the **Beta-entry hardening gate** below: nothing load-bearing crosses into real-data use until that gate is closed.
...
- **Rotate the secrets that appeared in a chat transcript** — `SERVICE_ROLE_KEY`, `ANON_KEY`, `RESEND_API_KEY`, `CRON_SHARED_SECRET`. The service-role key is highest priority: it bypasses RLS, so until it is rotated, every other control built into this platform (RLS policies, tenant isolation, the audit log) is decorative. Rotate, then update `docs/secrets-inventory.md` and `project_secret_rotation_status.md` (the latter still reads rotated-done 2026-05-17, which is stale). **This is the one item not waivable by convenience.**
- **R-4 cross-tenant isolation verified manually** before the first real f&f user, or at second-account creation at the latest. The automated pgTAP version stays deferred until a second practitioner account exists.
- **A backup restore actually exercised once** — not "PITR is available on the plan," but a restore proven on a scratch project, documented in `docs/disaster-recovery.md`, even at f&f scale.
- The remaining technical pre-beta riders indexed in `docs/go-live-checklist.md` (the open `client_accept_invite` anon-EXECUTE verification; the post-deploy compat-shim removal) either closed or consciously accepted with a recorded re-trigger.
